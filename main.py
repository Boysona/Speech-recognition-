...
def handle_all_text_messages(message):
    ...
    if message.text.startswith('/'):
        # Let other command handlers take care of it
        return
    
    # If it's a text message and not a command, attempt TTS
    # This assumes that if a user sends plain text, they intend it for TTS
    # after having chosen a voice.
    # You might want a more explicit state management system for TTS if this causes issues.
    
    # Here, we will just pass the text to the TTS function
    # A more robust solution would involve setting a user state, e.g.,
    # if user_state[uid] == 'awaiting_tts_text':
    # This implementation will try to synthesize any non-command text.
    
    # It's better to make TTS explicitly triggered rather than handling all text
    # The original TTS bot only handled text if it wasn't a command.
    # To avoid conflict with general chatbot behavior (if any was intended for Gemini),
    # we'll stick to the explicit command approach for TTS.
    
    # Removed the general text handler from the original TTS bot.
    # Users should use /text_to_speech and then send text.
