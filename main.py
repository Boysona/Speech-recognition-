...
import telebot
import json
import subprocess # Loogu talagalay in lagu beddelo os.system
...
# --- BOT CONFIGURATION ---
# Waxaan kor u qaadnay TOKEN, ADMIN_ID, iyo WEBHOOK_URL
...
# Download directory (waxaa loo isticmaali doonaa oo kaliya si ku meel gaar ah haddii in-memory ay adkaato)
...
# Waa in la nadiifiyaa si joogto ah (tusaale, 7 maalmood ka dib)
user_memory = {}
user_transcriptions = {}
...
def update_uptime_message(chat_id, message_id, stop_event):
    ...
    while not stop_event.is_set():
        try:
            ...
            break # Stop thread if message is deleted or inaccessible
        except Exception as e:
            logging.error(f"Unexpected error in uptime thread: {e}")
            break

# Nadiifinta xusuusta ku meel gaarka ah (user_transcriptions, user_memory)
def clean_old_user_data():
    """
    Nadiifi xogta isticmaalaha ee duugowday (transcriptions iyo memory) 7 maalmood ka dib.
    Waa in lagu daraa bilawga bot-ka ama jadwal joogto ah.
    """
    ...
    for user_id, transcriptions in user_transcriptions.items():
        # Tani waxay u baahan tahay hab wax ku ool ah oo lagu keydiyo timestamp-ka transcription kasta
        # Si fudud, waxaan u tirtiri karnaa dhammaan transcriptions-ka isticmaalaha haddii isticmaalaha uusan firfircoonayn.
        # Hadii aan u baahanahay mid sax ah, waa inaan ku darnaa timestamp transcription kasta.
        pass # Waxaan u dhaafayaa tan sida ay hadda tahay, laakiin maskaxda ku hay.

    keys_to_delete_memory = []
    for user_id, history in user_memory.items():
        # Tani way adag tahay in si toos ah loo maareeyo iyada oo aan timestamp ku jirin qayb kasta.
        # Habka ugu fiican ayaa ah in la tirtiro haddii user_data uusan firfircoonayn.
    ...
    for user_id in keys_to_delete_memory:
        ...
        # Hadii user_transcriptions sidoo kale aan lagu xirin message_id gaar ah, sidoo kale waan tirtiri karnaa.
    ...
    # Waxaan ku celcelin karnaa tan tusaale, 24 saac kasta
        ...
        with admin_uptime_lock:
            ...
            if ADMIN_ID in admin_uptime_message and admin_uptime_message[ADMIN_ID].get('stop_event'):
                ...
                if admin_uptime_message[ADMIN_ID].get('thread') and admin_uptime_message[ADMIN_ID]['thread'].is_alive():
                    admin_uptime_message[ADMIN_ID]['thread'].join(timeout=2) # Suggest stopping
...
def keep_typing(chat_id, stop_event):
    """Kani wuxuu diraa 'typing' action si joogto ah ilaa stop_event la dejiyo."""
    while not stop_event.is_set():
        try:
            bot.send_chat_action(chat_id, 'typing')
            time.sleep(4) # Telegram waxay u baahan tahay in la diro 5 ilbiriqsi kasta
        except Exception as e:
    ...
    try:
        ...
        if uid not in user_media_language_settings:
            bot.send_message(chat_id,
                             "⚠️ Fadlan marka hore dooro luqadda feylka maqalka ah adoo isticmaalaya /media_language ka hor inta aadan dirin feylka.")
            return
        ...
        if file_obj.file_size > FILE_SIZE_LIMIT:
            bot.send_message(chat_id, "Feylka aad soo dirtay cabbirkiisu wuu ka weyn yahay xadka loo ogol yahay (ugu badnaan 20MB).")
            return
        ...
        # Convert to WAV in-memory using pydub and ffmpeg (via subprocess)
        try:
            audio_segment = AudioSegment.from_file(audio_in_memory)
            wav_in_memory = BytesIO()
            audio_segment.export(wav_in_memory, format="wav", parameters=["-acodec", "pcm_s16le", "-ar", "16000", "-ac", "1"])
            ...
            bot.send_message(chat_id, "😓 Waan ka xunnahay, feylkaaga lama beddeli karo qaabka saxda ah ee aqoonsiga codka. Fadlan isku day feyl kale.")
            return

        media_lang_code = get_lang_code(user_media_language_settings[uid])
        if not media_lang_code:
            bot.send_message(chat_id, f"❌ Luqadda *{user_media_language_settings[uid]}* ma lahan code sax ah. Fadlan dib u dooro luqadda.")
            return
        ...
        if len(transcription) > 4000:
            ...
            with open(fn, 'rb') as doc:
                bot.send_document(
                    ...
                    caption="Halkan waxaa ah transcription-kaaga. Guji badhamada hoose si aad u hesho fursado kale."
                )
            os.remove(fn)
        else:
            bot.reply_to(
        ...
        # Error handling oo soo jiidasho leh
        error_message = "😓 Waan ka xunnahay, khalad lama filaan ah ayaa dhacay intii aan feylkaaga farsameynayay."
        if "Bad audio" in str(e) or "empty data" in str(e):
            error_message = "😓 Waan ka xunnahay, codka feylkaaga ma cadda ama ma dhameystirna. Fadlan isku day feyl cod cad leh."
        elif "No transcription" in str(e) or "cannot understand" in str(e):
            error_message = "😓 Waan ka xunnahay, ma fahmin waxa ku jira codkaaga. Fadlan hubi in codku cad yahay oo luqaddaadu ay sax tahay."
        bot.send_message(chat_id, error_message)
    finally:
        stop_typing_event.set() # Jooji typing action
        # Nadiifinta faylasha ku meel gaarka ah haddii ay jiraan (in-memory ma u baahna nadiifin)
        # Horey ayay u tirtireen haddii disk loo isticmaalo.

@bot.message_handler(content_types=['voice', 'audio', 'video', 'video_note'])
def handle_file(message):
    # Bilaw processing in a separate thread
...
def select_language_command(message):
    ...
    bot.send_message(
        message.chat.id,
        "Fadlan dooro luqadda aad door bidayso ee **turjumaadaha iyo soo koobidda** mustaqbalka:",
        reply_markup=markup
    )
...
def callback_set_language(call):
    ...
    bot.edit_message_text(
        ...
        text=f"✅ Luqadda aad door bidayso ee turjumaadaha iyo soo koobidda waa la dejiyay: **{lang}**",
        parse_mode="Markdown"
    )
    bot.answer_callback_query(call.id, text=f"Luqadda waxaa loo dejiyay {lang}")
...
def select_media_language_command(message):
    ...
    bot.send_message(
        message.chat.id,
        "Fadlan dooro luqadda feylasha maqalka ah ee aad u baahan tahay inaan u turjumo. Tani waxay caawisaa in la hubiyo akhris sax ah.",
        reply_markup=markup
    )
...
def callback_set_media_language(call):
    ...
    bot.edit_message_text(
        chat_id=call.message.chat.id,
        message_id=call.message.message_id,
        text=f"✅ Luqadda turjumaadda ee media-kaaga waa la dejiyay: **{lang}**",
        parse_mode="Markdown"
    )
    bot.answer_callback_query(call.id, text=f"Luqadda media-ga waxaa loo dejiyay {lang}")
...
def button_translate_handler(call):
    ...
    if uid not in user_transcriptions or message_id not in user_transcriptions[uid]:
        bot.answer_callback_query(call.id, "❌ Ma jiro transcription laga helay fariintan.")
        return

    preferred_lang = user_language_settings.get(uid)
    if preferred_lang:
        bot.answer_callback_query(call.id, "Ku turjumaya luqadda aad door bidayso...")
        ...
        bot.edit_message_text(
            chat_id=call.message.chat.id,
            message_id=call.message.message_id,
            text="Fadlan dooro luqadda aad rabto inaad ku turjunto:",
            reply_markup=markup
        )
...
def button_summarize_handler(call):
    ...
    preferred_lang = user_language_settings.get(uid)
    if preferred_lang:
        bot.answer_callback_query(call.id, "Ku soo koobaya luqadda aad door bidayso...")
        ...
        bot.edit_message_text(
            chat_id=call.message.chat.id,
            message_id=call.message.message_id,
            text="Fadlan dooro luqadda aad rabto in soo koobidda lagu sameeyo:",
            reply_markup=markup
        )
    bot.answer_callback_query(call.id)
...
def callback_translate_to(call):
    ...
    # Waxaan ku cusboonaysiineynaa fariintii hore in la turjumayo
    bot.edit_message_text(
        chat_id=call.message.chat.id,
        message_id=call.message.message_id,
        text=f"Ku turjumaya **{lang}**...",
        parse_mode="Markdown"
    )
    ...
    if message_id:
        ...
        if uid in user_transcriptions and call.message.reply_to_message and call.message.reply_to_message.message_id in user_transcriptions[uid]:
            ...
            bot.send_message(call.message.chat.id, "❌ Ma jiro transcription laga helay fariintan oo la turjumo. Fadlan isticmaal badhamada inline-ka ee transcription-ka.")
    bot.answer_callback_query(call.id)
...
def callback_summarize_in(call):
    ...
    # Waxaan ku cusboonaysiineynaa fariintii hore in la soo koobayo
    bot.edit_message_text(
        chat_id=call.message.chat.id,
        message_id=call.message.message_id,
        text=f"Ku soo koobaya **{lang}**...",
        parse_mode="Markdown"
    )
    ...
    if message_id:
        ...
        if uid in user_transcriptions and call.message.reply_to_message and call.message.reply_to_message.message_id in user_transcriptions[uid]:
            ...
            bot.send_message(call.message.chat.id, "❌ Ma jiro transcription laga helay fariintan oo la soo koobo. Fadlan isticmaal badhamada inline-ka ee transcription-ka.")
    bot.answer_callback_query(call.id)

def do_translate_with_saved_lang(message, uid, lang, message_id):
    original = user_transcriptions.get(uid, {}).get(message_id, "")
    if not original:
        bot.send_message(message.chat.id, "❌ Ma jiro transcription diyaar ah oo la turjumo fariintan gaarka ah.")
        return
    ...
    if translated.startswith("Error:"):
        bot.send_message(message.chat.id, f"Fadlan isku day markale. Khalad ayaa dhacay intii lagu jiray turjumaadda. Waa kan qaladka: {translated}")
        return
    ...
    if len(translated) > 4000:
        ...
        with open(fn, 'rb') as doc:
            bot.send_document(message.chat.id, doc, caption=f"Turjumidda {lang}", reply_to_message_id=message_id)
        os.remove(fn)
    else:
...
def do_summarize_with_saved_lang(message, uid, lang, message_id):
    original = user_transcriptions.get(uid, {}).get(message_id, "")
    if not original:
        bot.send_message(message.chat.id, "❌ Ma jiro transcription diyaar ah oo la soo koobo fariintan gaarka ah.")
        return
    ...
    if summary.startswith("Error:"):
        bot.send_message(message.chat.id, f"Fadlan isku day markale. Khalad ayaa dhacay intii lagu jiray soo koobidda. Waa kan qaladka: {summary}")
        return
    ...
    if len(summary) > 4000:
        ...
        bot.send_chat_action(message.chat.id, 'upload_document')
        with open(fn, 'rb') as doc:
            bot.send_document(message.chat.id, doc, caption=f"Soo koobidda {lang}", reply_to_message_id=message_id)
        os.remove(fn)
    else:
...
def handle_translate(message):
    ...
    if not message.reply_to_message or uid not in user_transcriptions or message.reply_to_message.message_id not in user_transcriptions[uid]:
        return bot.send_message(message.chat.id, "❌ Fadlan uga jawaab fariin transcription ah si aad u turjunto.")
    ...
    if preferred_lang:
        ...
        bot.send_message(
            message.chat.id,
            "Fadlan dooro luqadda aad rabto inaad ku turjunto:",
            reply_markup=markup
        )
...
def handle_summarize(message):
    ...
    if not message.reply_to_message or uid not in user_transcriptions or message.reply_to_message.message_id not in user_transcriptions[uid]:
        return bot.send_message(message.chat.id, "❌ Fadlan uga jawaab fariin transcription ah si aad u soo koobto.")
    ...
    if preferred_lang:
        ...
        bot.send_message(
            message.chat.id,
            "Fadlan dooro luqadda aad rabto in soo koobidda lagu sameeyo:",
            reply_markup=markup
        )
...
def transcribe_audio_chunks_in_memory(audio_bytes_io: BytesIO, lang_code: str) -> str | None:
    r = sr.Recognizer()
    full_transcription = []
    chunk_length_ms = 20000  # 20 seconds per chunk (from 10 seconds)
    overlap_ms = 500

    try:
        audio = AudioSegment.from_file(audio_bytes_io, format="wav") # Assume input is already WAV
        ...
        while start_ms < total_length_ms:
            ...
            with sr.AudioFile(chunk_in_memory) as source:
                try:
                    ...
                    chunk_in_memory.close() # Close the BytesIO object for the chunk
...
def fallback(message):
    ...
    if message.text and message.text.startswith('/'):
        ...
        bot.send_message(message.chat.id, "Fadlan soo dir kaliya farriimaha codka, maqalka, ama fiidyowyada si loo turjumo.")
...
if __name__ == "__main__":
    ...
    # Bilaw nadiifinta xogta duugowday
    clean_old_user_data()
