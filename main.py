...
# --- NEW: Mock database for fingerprints and hashes ---
mock_fingerprints = set()
mock_hashes = set()

# --- Functions for fingerprinting and hashing ---
def get_fingerprint(file_path):
    try:
        # chromaprint.fingerprint_audio_file returns (duration, fingerprint_encoded)
...
def get_sha256_hash(audio_segment):
    try:
        ...
        logging.error(f"Error generating SHA256 hash for audio segment: {e}")
        return None
# --- END NEW FUNCTIONS ---
...
def process_media_file(message, stop_typing, is_document_video):
    ...
    convert it to WAV, run SpeechRecognition transcription (in chunks),
    ...
    try:
        ...
        # --- NEW: Transcribe using SpeechRecognition with chunking, fingerprinting, and hashing ---
...
# --- UPDATED: Transcribe in chunks using pydub + SpeechRecognition with fingerprinting and hashing ---
def transcribe_audio_with_chunks(audio_path: str, lang_code: str) -> str | None:
    ...
    try:
        ...
        if total_ms > 60_000:  # If more than 1 minute
            chunk_length_ms = 30_000  # 30 seconds
        else:
        ...
        for i in range(0, total_ms, chunk_length_ms):
            ...
            chunk_filename = os.path.join(
                DOWNLOAD_DIR,
                f"{uuid.uuid4()}_{i // 1000}_{min((i + chunk_length_ms) // 1000, total_ms // 1000)}.wav"
            )

            # Generate new UUID and SHA-256 hash for the current chunk
            chunk_uuid = str(uuid.uuid4())
            chunk_hash = get_sha256_hash(chunk)

            # --- NEW: Export chunk with metadata tags ---
            chunk.export(chunk_filename, format="wav", tags={
                "chunk_id": os.path.basename(chunk_filename), # A unique ID for the chunk file
                "db_id": chunk_uuid, # A new UUID for the "mock database" entry
                "hash": chunk_hash # The SHA-256 hash of the raw audio data
            })
            # --- END NEW ---

            # --- NEW: Check Fingerprint and Hash against mock database ---
            fp = get_fingerprint(chunk_filename)
            
            # If fingerprint exists, skip
            if fp and fp in mock_fingerprints:
                logging.info(f"Skipping chunk {chunk_filename}: Fingerprint already processed.")
                os.remove(chunk_filename)
                continue
            
            # If hash exists (and fingerprint didn't catch it, which is less likely for duplicates)
            if chunk_hash and chunk_hash in mock_hashes:
                logging.info(f"Skipping chunk {chunk_filename}: Hash already processed.")
                os.remove(chunk_filename)
                continue

            # Add new fingerprint and hash to mock databases
            if fp:
                mock_fingerprints.add(fp)
            if chunk_hash:
                mock_hashes.add(chunk_hash)
            # --- END NEW CHECK ---
            ...
            try:
                ...
                # Clean up the chunk file before returning None on error
                os.remove(chunk_filename) 
                return None
            except Exception as e:
...
LANGUAGES = [
    {"name": "English", "flag": "🇬🇧", "code": "en-US"}, # Changed to specific codes for SR where applicable
    ...
    {"name": "Chinese", "flag": "🇨🇳", "code": "zh-CN"}, # Mandarin
    {"name": "Japanese", "flag": "🇯🇵", "code": "ja-JP"},
    {"name": "Portuguese", "flag": "🇵🇹", "code": "pt-PT"}, # Portugal Portuguese
    ...
    {"name": "Filipino", "flag": "🇵🇭", "code": "fil-PH"}, # Tagalog (Filipino)
    ...
    {"name": "Norwegian", "flag": "🇳🇴", "code": "nb-NO"}, # Bokmål (Norwegian)
