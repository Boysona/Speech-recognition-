...
TOKEN = "7790991731:AAGpbz6nqE5f0Dvs6ZSTpRoR1LMrrf4rMqU"  # Replace with your actual bot token
...
DB_NAME = "telegram_bot_db"  # You can choose a name for your database
...
STATISTICS_COLLECTION = "bot_statistics"
...
bot_statistics_collection = None # New collection for statistics

def connect_to_mongodb():
    global mongo_client, db, users_collection, language_settings_collection, media_language_settings_collection, tts_users_collection, bot_statistics_collection
    try:
        ...
        # --- NEW: Initialize statistics collection ---
        bot_statistics_collection = db[STATISTICS_COLLECTION]
        # Ensure the statistics document exists and is initialized
        initialize_bot_statistics()
        logging.info("Successfully connected to MongoDB!")
    except ConnectionFailure as e:
        logging.error(f"MongoDB connection failed: {e}")
        exit(1)  # Stop if DB connection fails
...
# --- NEW: Functions for persistent bot statistics ---
def initialize_bot_statistics():
    """Initializes the bot statistics document in MongoDB if it doesn't exist."""
    try:
        if bot_statistics_collection.count_documents({}) == 0:
            bot_statistics_collection.insert_one({
                "_id": "global_stats",
            ...
            logging.info("Initialized bot statistics in MongoDB.")
    except Exception as e:
        logging.error(f"Error initializing bot statistics: {e}")

def get_bot_statistics():
    """Fetches global bot statistics from MongoDB."""
    try:
        stats = bot_statistics_collection.find_one({"_id": "global_stats"})
        if stats:
            return stats
        return {
            "total_files_processed": 0,
            ...
            "total_videos": 0,
            "total_processing_time": 0.0,
            "bot_start_time": datetime.now().isoformat() # Fallback for new init
        }
    except Exception as e:
        logging.error(f"Error fetching bot statistics: {e}")
        return {
            "total_files_processed": 0,
            "total_audio_files": 0,
            "total_voice_clips": 0,
...
def update_bot_statistics(file_type: str, processing_duration: float):
    """
    Updates global bot statistics in MongoDB.
    :param file_type: 'audio', 'voice', or 'video'
    :param processing_duration: Time taken for processing in seconds.
    """
    try:
        update_doc = {
            "$inc": {
        ...
        if file_type == 'audio':
            update_doc["$inc"]["total_audio_files"] = 1
        elif file_type == 'voice':
            update_doc["$inc"]["total_voice_clips"] = 1
        elif file_type == 'video':
            update_doc["$inc"]["total_videos"] = 1

        bot_statistics_collection.update_one(
            {"_id": "global_stats"},
            update_doc,
            upsert=True
        )
        logging.info(f"Updated global bot statistics for {file_type} processing.")
    except Exception as e:
        logging.error(f"Error updating global bot statistics: {e}")
...
# Removed global statistics counters as they will now be fetched from DB
# total_files_processed = 0
# total_audio_files = 0
# total_voice_clips = 0
# total_videos = 0
# total_processing_time = 0
# bot_start_time = datetime.now() # This will now be fetched from DB
...
# Replaced local save_user_data with DB update
...
def update_uptime_message(chat_id, message_id):
    ...
    """
    global bot_start_time # Use global to get the start time from DB, or current if not initialized

    stats = get_bot_statistics()
    bot_start_time_str = stats.get("bot_start_time")
    if bot_start_time_str:
        bot_start_time_dt = datetime.fromisoformat(bot_start_time_str)
    else:
        bot_start_time_dt = datetime.now() # Fallback if for some reason it's not in DB

    while True:
        try:
            elapsed = datetime.now() - bot_start_time_dt
...
Should you have any questions or concerns regarding your privacy, please feel to contact the bot administrator.
"""
    )
...
def status_handler(message):
    ...
    total_audio_files = stats.get("total_audio_files", 0)
    total_voice_clips = stats.get("total_voice_clips", 0)
    total_videos = stats.get("total_videos", 0)
    total_processing_time = stats.get("total_processing_time", 0.0)
    bot_start_time_str = stats.get("bot_start_time", datetime.now().isoformat()) # Fallback for new init
    bot_start_time = datetime.fromisoformat(bot_start_time_str)
    ...
    # Fetch active users today from MongoDB
    try:
...
def handle_file(message):
    ...
    file_obj = None
    file_type_for_stats = None # To store 'audio', 'voice', or 'video' for statistics
    if message.voice:
        file_obj = message.voice
        file_type_for_stats = 'voice'
    elif message.audio:
        file_obj = message.audio
        file_type_for_stats = 'audio'
    elif message.video:
        file_obj = message.video
        file_type_for_stats = 'video'
    elif message.video_note:
        file_obj = message.video_note
        file_type_for_stats = 'video' # Treat video_note as video for stats
    ...
    try:
        threading.Thread(
            ...
            args=(message, stop_typing, file_type_for_stats) # Pass file_type_for_stats
        ).start()
    except Exception as e:
...
def process_media_file(message, stop_typing, file_type_for_stats):
    """
    ...
    """
    # Removed global references, now update_bot_statistics handles it
    ...
    if message.voice:
        ...
        file_obj = message.video_note
    else:  # is_document_video == True or audio document
        file_obj = message.document

    local_temp_file = None
    wav_audio_path = None

    processing_start_time = datetime.now() # Start timing here
    ...
    try:
        ...
        media_lang_name = get_user_media_language_setting_db(uid)  # e.g. "English"
        ...
        update_bot_statistics(file_type_for_stats, processing_time)
        ...
        # --- Check and send subscription message AFTER successful transcription ---
...
# --- Transcribe in 20-second chunks using pydub + SpeechRecognition ---
...
async def synth_and_send_tts(chat_id, user_id, text):
    voice = get_tts_user_voice_db(user_id)  # Use DB function
...
def button_translate_handler(call):
    ...
    preferred_lang = get_user_language_setting_db(uid)  # Use DB function
    if preferred_lang:
...
def handle_text_for_tts_or_fallback(message):
    ...
    elif get_tts_user_voice_db(uid) != "en-US-AriaNeural":  # Check if it's not the default voice
        user_tts_mode[uid] = get_tts_user_voice_db(uid)  # Reactivate TTS mode with the saved voice
        threading.Thread(
            target=lambda: asyncio.run(synth_and_send_tts(message.chat.id, uid, message.text))
        ).start()
    else:
...
def cleanup_old_data():
    """Cleans up user_transcriptions and user_memory older than 7 days from in-memory and MongoDB."""
    seven_days_ago_iso = (datetime.now() - timedelta(days=7)).isoformat()

    # Cleanup in-memory transcriptions and user memory
    ...
    # Reset transcription count for very inactive users (> 30 days)
    ...
    # Cleanup orphaned language and TTS settings
    try:
