...
TOKEN = "7790991731:AAH4rt8He_PABDa28xgcY3dIQwmtuQD-qiM"  # Beddel token-kaaga bot-ka ee dhabta ah  
ADMIN_ID = 5978150981  # Beddel ID-gaaga Admin-ka ee dhabta ah  
# Webhook URL - Beddel URL-ga Render-kaaga ee dhabta ah  
WEBHOOK_URL = "https://speech-recognition-9cyh.onrender.com"  

# --- REQUIRED CHANNEL CONFIGURATION ---  
REQUIRED_CHANNEL = "@transcriberbo"  # Beddel magaca channel-kaaga ee dhabta ah  
...
GEMINI_API_KEY = "AIzaSyAto78yGVZobxOwPXnl8wCE9ZW8Do2R8HA"  # Beddel Gemini API Key-gaaga  
...
def check_subscription(user_id):  
    if not REQUIRED_CHANNEL:  
        return True  # Waayahan channel la rabin, markasta True bixiya  
...
def send_subscription_message(chat_id):  
    if not REQUIRED_CHANNEL:  
        return  # Haddii channel la rabin, waxba ha sameynin  
...
def requires_subscription(user_id):  
    """  
    Return True if we need to enforce subscription: i.e., user has completed five (5) or more transcriptions.  
    """  
...
def start_handler(message):  
    ...
    if message.from_user.id == ADMIN_ID:  
        ...
        # --- MODIFIED: Delay subscription prompt until after five (5) transcriptions ---  
...
4.,  **Data Retention:**  
    ...
    * **Transcriptions:** Held temporarily in the bot's active memory for immediate use and cleared after seven (7) days or when superseded.,  
...
def transcribe_audio_with_speech_recognition(audio_path: str, lang_code: str) -> str | None:  
    ...
    try:  
        with sr.AudioFile(audio_path) as source:  
            ...
            # Use Google Web Speech API (free, but has limits)  
        ...
        logging.error(f"SpeechRecognition error: {e}")  
        return None  

# --- Memory Cleanup Function ---  
def cleanup_old_data():  
    """Cleans up user_transcriptions and user_memory older than seven (7) days."""  
