...
# Note: We’ll persist per-file stats in MongoDB; these counters are kept for quick in-memory access if needed.
...
def status_handler(message):
    ...
    # Fetch active users today from MongoDB (based on last_active >= today)
    try:
    ...
    # Fetch processing statistics from MongoDB
    try:
...
def process_media_file(message, stop_typing, is_document_video, type_str):
    ...
    store processing stats in MongoDB, and send back the result.
    """
    ...
    try:
        ...
        # Record success stat in MongoDB
        try:
            processing_stats_collection.insert_one({
                "user_id": uid,
                "message_id": message.message_id,
        ...
        # Record failure stat
        processing_time = (datetime.now() - datetime.strptime(message.date and message.date, "%Y-%m-%dT%H:%M:%S.%fZ")).total_seconds() if message.date else 0
        try:
            processing_stats_collection.insert_one({
                "user_id": uid,
                "message_id": message.message_id,
...
def cleanup_old_data():
    ...
    try:
        ...
        logging.info("Cleaned up orphaned language, TTS, and processing stats settings.")
    except Exception as e:
