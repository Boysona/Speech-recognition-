...
# --- START: REQUIRED_CHANNEL Implementation ---
REQUIRED_CHANNEL = "@transcriberbo" # Replace with your actual channel username (e.g., @mytranscriberbotchannel)

def check_subscription(user_id):
    if user_id == ADMIN_ID: # Admin can always use the bot
        return True
    try:
        ...
        # This error usually means the user is not found in the chat (i.e., not a member)
        logging.warning(f"User {user_id} not found in channel {REQUIRED_CHANNEL}: {e}")
        return False
...
def send_subscription_message(chat_id):
    ...
    bot.send_message(
        chat_id,
        f"👋🏻 To use this bot, you must first join our channel: {REQUIRED_CHANNEL}. Please join the channel, then come back to use the bot.🥰",
        reply_markup=markup,
        parse_mode="Markdown"
    )
# --- END: REQUIRED_CHANNEL Implementation ---
...
def start_handler(message):
    ...
    # --- START: Check REQUIRED_CHANNEL at start ---
    if not check_subscription(message.from_user.id):
        send_subscription_message(message.chat.id)
        return
    # --- END: Check REQUIRED_CHANNEL at start ---
...
def help_handler(message):
    # --- START: Check REQUIRED_CHANNEL ---
    if not check_subscription(message.from_user.id):
        send_subscription_message(message.chat.id)
        return
    # --- END: Check REQUIRED_CHANNEL ---
...
def handle_file(message):
    ...
    # --- START: Check REQUIRED_CHANNEL for media handling ---
    if not check_subscription(message.from_user.id):
        send_subscription_message(message.chat.id)
        return
    # --- END: Check REQUIRED_CHANNEL for media handling ---
