...
# Should be cleaned regularly (e.g., after 7 days)
user_memory = {}
user_transcriptions = {}
...
# --- Admin uptime/message tracking (wajibaadka saxa)
admin_uptime_lock = threading.Lock()
admin_uptime_message = {}  # qaabka: {admin_id: {'chat_id': ..., 'message_id': ..., 'thread': thread_object}}

def update_uptime_message(chat_id, message_id):
    """
    Function to be run in a separate thread, 
    si uu xogta uptime u cusbooneysiiyo farriin admin-ka ah.
    """
    while True:
        with admin_uptime_lock:
            # Haddii farriinta la tirtiro, ama chat_id/message_id la beddelo, jooji
            if admin_uptime_message.get(ADMIN_ID, {}).get('chat_id') != chat_id or \
               admin_uptime_message.get(ADMIN_ID, {}).get('message_id') != message_id:
                break
            ...
            text = f"Admin Panel and Uptime (updating live)...\n\n" \
                   f"⏳ Uptime: {days}d {hours}h {minutes}m {seconds}s"
            try:
                bot.edit_message_text(chat_id=chat_id, message_id=message_id, text=text)
            except Exception as e:
                logging.error(f"Failed to update uptime message: {e}")
                break

        # Cusbooneysii 60 ilbiriqsi kasta
        time.sleep(60)
...
def update_user_activity(user_id: int):
    # Key-ga user_data waa string-ka user_id
...
def clean_old_user_data():
    ...
    for user_id_str, history in user_memory.items():
        last_active_str = user_data.get(user_id_str)
        if last_active_str:
    ...
    for user_id_str in keys_to_delete_memory:
        if user_id_str in user_memory:
            ...
            logging.info(f"Cleaned memory for inactive user: {user_id_str}")
        if user_id_str in user_transcriptions:
            del user_transcriptions[user_id_str]
            logging.info(f"Cleaned transcriptions for inactive user: {user_id_str}")
...
def start_handler(message):
    ...
    update_user_activity(user_id_int)
    ...
    if message.from_user.id == ADMIN_ID:
        # Admin panel: soo bandhig keyboard
        ...
        with admin_uptime_lock:
            ...
            # Ku abuur thread si loo cusbooneysiiyo uptime
        ...
        display_name = message.from_user.first_name or (
            ...
            if message.from_user.username else "user"
        )
...
def broadcast_message(message):
    ...
    for uid_key in user_data:
        try:
            bot.copy_message(int(uid_key), message.chat.id, message.message_id)
            success += 1
...
def process_media_file_threaded(message):
    ...
    try:
        update_user_activity(uid_int)

        if uid_str not in user_media_language_settings:
            bot.send_message(
                chat_id,
        ...
        try:
            if message.voice or message.audio or message.video or message.video_note:
                # Qaar ka mid ah versionnada Telebot ma aqbalaan set_message_reaction,
                # sidaas awgeed waa la hubiyaa in haddii khalad yimaado waxba noqon mayso.
        ...
        media_lang_code = get_lang_code(user_media_language_settings[uid_str])
        if not media_lang_code:
            bot.send_message(
                chat_id,
                f"❌ The language *{user_media_language_settings[uid_str]}* does not have a valid code. Please choose again."
            )
            return

        transcription = transcribe_audio_chunks_in_memory(wav_in_memory, media_lang_code) or ""
        user_transcriptions.setdefault(uid_str, {})[message_id] = transcription
        ...
        # Tixraac khaladaadka caadiga ah ee SpeechRecognition
...
def callback_set_language(call):
    ...
    update_user_activity(user_id_int)
    # Kala qaybi hal jeer si magaca luqadda uusan u lumin format ka
    _, lang = call.data.split("|", 1)
    user_language_settings[str(user_id_int)] = lang
...
def callback_set_media_language(call):
    ...
    update_user_activity(user_id_int)
    _, lang = call.data.split("|", 1)
    user_media_language_settings[str(user_id_int)] = lang
...
def button_translate_handler(call):
    ...
    if uid_str not in user_transcriptions or message_id not in user_transcriptions[uid_str]:
        bot.answer_callback_query(call.id, "❌ No transcription found for this message.")
        return

    preferred_lang = user_language_settings.get(uid_str)
    if preferred_lang:
        bot.answer_callback_query(call.id, "Translating to your preferred language...")
        threading.Thread(
            target=do_translate_with_saved_lang,
            args=(call.message, user_id_int, preferred_lang, message_id)
        ).start()
    else:
...
def callback_translate_to(call):
    ...
    # parts[0] = "translate_to", parts[1] = luqadda, parts[2] = message_id (haddii jira)
    lang = parts[1]
    message_id = int(parts[2]) if len(parts) > 2 else None
    uid_str = str(user_id_int)

    user_language_settings[uid_str] = lang
    save_user_language_settings()
    ...
    if message_id:
        threading.Thread(
            target=do_translate_with_saved_lang,
            args=(call.message, user_id_int, lang, message_id)
        ).start()
    else:
        # Xaaladda fallback, fiiri haddii fariinta la reply gareeyey
        if uid_str in user_transcriptions and call.message.reply_to_message \
           and call.message.reply_to_message.message_id in user_transcriptions[uid_str]:
            threading.Thread(
                target=do_translate_with_saved_lang,
                args=(call.message, user_id_int, lang, call.message.reply_to_message.message_id)
            ).start()
        else:
            bot.send_message(
                call.message.chat.id,
                "❌ No transcription found to translate. Please use the inline buttons on the transcription message."
            )
    bot.answer_callback_query(call.id)
...
def callback_summarize_in(call):
    ...
    update_user_activity(user_id_int)
    parts = call.data.split("|")
    # parts[0] = "summarize_in", parts[1] = luqadda, parts[2] = message_id (haddii jira)
    lang = parts[1]
    message_id = int(parts[2]) if len(parts) > 2 else None
    uid_str = str(user_id_int)
    ...
    if message_id:
        ...
        if uid_str in user_transcriptions and call.message.reply_to_message \
           and call.message.reply_to_message.message_id in user_transcriptions[uid_str]:
            ...
            bot.send_message(
                call.message.chat.id,
                "❌ No transcription found to summarize. Please use the inline buttons on the transcription message."
            )
    bot.answer_callback_query(call.id)

def do_translate_with_saved_lang(message, user_id_int: int, lang: str, message_id: int):
    uid_str = str(user_id_int)
    ...
    translated = ask_gemini(user_id_int, prompt)
...
def do_summarize_with_saved_lang(message, user_id_int: int, lang: str, message_id: int):
    uid_str = str(user_id_int)
    original = user_transcriptions.get(uid_str, {}).get(message_id, "")
    if not original:
    ...
    bot.send_chat_action(message.chat.id, 'typing')
    summary = ask_gemini(user_id_int, prompt)
...
def handle_translate(message):
    ...
    if not message.reply_to_message or uid_str not in user_transcriptions \
       or message.reply_to_message.message_id not in user_transcriptions[uid_str]:
    ...
    if preferred_lang:
        threading.Thread(
            target=do_translate_with_saved_lang,
            args=(message, user_id_int, preferred_lang, transcription_message_id)
        ).start()
    else:
...
def transcribe_audio_chunks_in_memory(audio_bytes_io: BytesIO, lang_code: str) -> str | None:
    ...
    try:
        ...
        logging.info(f"Starting in-memory chunking, total length {total_length_ms / 1000:.2f} seconds.")
        ...
        while start_ms < total_length_ms:
            ...
            with sr.AudioFile(chunk_in_memory) as source:
                try:
                    ...
                    logging.info(f"Transcribed chunk {start_ms/1000:.1f}s–{end_ms/1000:.1f}s: {text[:50]}...")
                except sr.UnknownValueError:
                    logging.warning(f"Speech Recognition could not understand audio in chunk {start_ms/1000:.1f}s–{end_ms/1000:.1f}s")
                except sr.RequestError as e:
                    logging.error(f"Could not request results from Google Speech Recognition service; {e} for chunk {start_ms/1000:.1f}s–{end_ms/1000:.1f}s")
                except Exception as e:
                    logging.error(f"Error processing chunk {start_ms/1000:.1f}s–{end_ms/1000:.1f}s: {e}")
                finally:
...
def fallback(message):
    ...
    if message.text and message.text.startswith('/'):
        # Haddii ay tahay amarka, iska dhaaf si handler-kii hore u qabto
        pass
    else:
...
if __name__ == "__main__":
    # Nadiifi folder-ka DOWNLOAD_DIR haddii uu horay u jiray
