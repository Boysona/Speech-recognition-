...
# --- REMOVE: user_media_language_settings_file will be removed as detection is automatic ---
...
try:
    # Set device to 'cpu' for broader compatibility if GPU is not guaranteed
    # 'int8' for compute_type is a good balance for speed and memory on CPU
...
# --- REMOVE: save_user_media_language_settings as detection is automatic ---
...
def set_bot_info():
    commands = [
        ...
        # --- REMOVE: media_language command as detection is automatic ---
...
1.  **Send a File for Transcription:**
    ...
    * The bot will **automatically detect the language** of the audio and transcribe it.
...
3.  **Commands:**
    ...
    * `/language`: Change your preferred language for translations and summaries. This setting applies to text outputs, not the original media.
    # --- REMOVED: media_language command from help ---
...
1.  **Data We Process & Its Lifecycle:**
    ...
    * **Language Preferences:** Your chosen languages for translations/summaries and your chosen voice for text-to-speech are saved. This ensures you don't need to re-select them for every interaction, making your experience smoother.
    # --- REMOVED: media transcription language preference from privacy notice ---
    # * Your chosen language for media transcription is saved.
...
def handle_file(message):
    ...
    # --- REMOVE: No need to prompt for media_language anymore as it's automatic ---
...
def process_media_file(message, stop_typing, is_document_video):
    ...
    convert it to WAV, run FasterWhisper transcription with ALD,
    ...
    try:
        ...
        # --- MODIFIED: Transcribe using FasterWhisper with Automatic Language Detection ---
        ...
        try:
            # We use a small initial segment to detect language
            audio_segment = AudioSegment.from_wav(wav_audio_path)
            # Take the first 30 seconds for language detection to be quick and accurate
            # Use raw_audio for detection as it expects numpy array
            _, info = WHISPER_MODEL.transcribe(
                audio_segment.set_frame_rate(16000).set_channels(1).raw_data, # ensure 16kHz mono for detection
                word_timestamps=False, # We don't need word timestamps for just detection
                vad_filter=True # Use voice activity detection to skip silence for better detection
            )
            detected_language = info.language
            detected_language_name = next((lang['name'] for lang in LANGUAGES if lang['code'] == detected_language), detected_language)
            logging.info(f"Detected language for transcription: {detected_language_name} ({detected_language})")

            # Now, perform the full transcription with the detected language
            segments, _ = WHISPER_MODEL.transcribe(wav_audio_path, language=detected_language)
            transcription = " ".join([segment.text for segment in segments])
        except Exception as e:
            logging.error(f"FasterWhisper transcription or language detection error: {e}")
            try:
                bot.set_message_reaction(chat_id=message.chat.id, message_id=message.message_id, reaction=[])
            except Exception as remove_e:
                logging.error(f"Error removing reaction on Whisper error: {remove_e}")
            bot.send_message(
                message.chat.id,
                "😓 Sorry, there was an issue during speech recognition or language detection. "
                "The audio might be too short, noisy, or contain no speech. Please try again."
            )
            return
        # --- END MODIFIED ---
        ...
        if len(transcription) > 4000:
            ...
            with open(fn, 'rb') as doc:
                bot.send_document(
                    ...
                    caption=f"Here’s your transcription (Detected Language: {detected_language_name}). Tap a button below for more options."
                )
            os.remove(fn)
        else:
            bot.reply_to(
                message,
                f"Transcription (Detected Language: {detected_language_name}):\n\n{transcription}",
                reply_markup=buttons
            )
        ...
        # --- FIXED: Use a triple-quoted string here to avoid unterminated literal ---
        bot.send_message(
            message.chat.id,
...
# --- REMOVED: media_language command and its callbacks are no longer needed ---
...
# --- MODIFIED: Function to transcribe audio using FasterWhisper with ALD ---
def transcribe_audio_with_faster_whisper(audio_path: str, lang_code: str = None) -> (str | None, str | None):
    """
    Transcribes audio using FasterWhisper. If lang_code is not provided,
    ...
    try:
        if lang_code is None:
            # Perform language detection if no language is specified
            # Load a small portion of the audio for detection (e.g., first 30 seconds)
            # Use pydub to load and segment the audio for detection
            audio_segment = AudioSegment.from_wav(audio_path)
            # Take a maximum of 30 seconds for detection
            detection_audio = audio_segment[:30000] # milliseconds
            
            # Export the small segment to a temporary WAV file for detection
            temp_detection_path = os.path.join(DOWNLOAD_DIR, f"detection_temp_{uuid.uuid4()}.wav")
            detection_audio.export(temp_detection_path, format="wav", parameters=["-acodec", "pcm_s16le", "-ar", "16000", "-ac", "1"])

            segments, info = WHISPER_MODEL.transcribe(
                temp_detection_path,
                word_timestamps=False, # No need for word timestamps for detection
                vad_filter=True # Use voice activity detection to skip silence for better detection
            )
            detected_language = info.language
            os.remove(temp_detection_path) # Clean up temporary detection file
            logging.info(f"Automatically detected language: {detected_language}")
            final_lang_code = detected_language
        else:
            final_lang_code = lang_code
            logging.info(f"Using specified language: {final_lang_code}")

        # Perform the full transcription with the detected/specified language
        segments, _ = WHISPER_MODEL.transcribe(audio_path, language=final_lang_code)
        full_transcription = " ".join([segment.text for segment in segments])

        return full_transcription, final_lang_code
