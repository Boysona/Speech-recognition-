...
Also, it can convert your text into speech!
...
def update_user_activity(user_id):
    ...
    if str_user_id not in user_data:
        user_data[str_user_id] = {}
    user_data[str_user_id]['last_activity'] = datetime.now().isoformat()
    # Initialize transcription_count if it doesn't exist
    if 'transcription_count' not in user_data[str_user_id]:
        user_data[str_user_id]['transcription_count'] = 0
    if 'channel_prompt_sent' not in user_data[str_user_id]:
        user_data[str_user_id]['channel_prompt_sent'] = False
    save_user_data()
...
def start_handler(message):
    ...
    if message.from_user.id == ADMIN_ID:
        ...
        # Instead of welcome message, send language selection buttons for the welcome language
        markup = generate_language_keyboard("welcome_lang")
        bot.send_message(
            message.chat.id,
            "👋🏻 Welcome! Please choose your preferred language to get started:",
            reply_markup=markup
        )

@bot.callback_query_handler(func=lambda c: c.data.startswith("welcome_lang|"))
def callback_welcome_language(call):
    uid = str(call.from_user.id)
    update_user_activity(uid) # Update activity before proceeding

    _, lang = call.data.split("|", 1)
    user_language_settings[uid] = lang # Save as preferred language
    save_user_language_settings()

    display_name = call.from_user.first_name or (f"@{call.from_user.username}" if call.from_user.username else "user")
...
"""
    ...
    bot.answer_callback_query(call.id, text=f"Language set to {lang}. Welcome!")
...
def help_handler(message):
    ...
    # The subscription check will be performed *after* the 5th transcription for new users
    # For existing users, this check might still be useful if they haven't subscribed.
    # However, for consistency with the new logic, we'll rely on the delayed prompt.
    # If a user is not subscribed and *tries to use the bot*, they will get the message.
    # This specific command does not count as a "transcription process".
    # So, no check_subscription here for help command.
...
def privacy_notice_handler(message):
    user_id = str(message.from_user.id)
    update_user_activity(user_id)
    # No check_subscription here as it's an informational command
...
def status_handler(message):
    ...
    active_today = sum(
        1 for uid_data in user_data.values()
        if 'last_activity' in uid_data and datetime.fromisoformat(uid_data['last_activity']).date() == today
    )
...
def process_media_file(message, stop_typing, is_document_video):
    ...
    try:
        ...
        media_lang_name = user_media_language_settings.get(uid)
        if not media_lang_name:
            # If user hasn't chosen a media_language yet, prompt them and return
            try:
                bot.set_message_reaction(chat_id=message.chat.id, message_id=message.message_id, reaction=[])
            except Exception as remove_e:
                logging.error(f"Error removing reaction on language selection prompt: {remove_e}")
            bot.send_message(
                message.chat.id,
        ...
        # Increment transcription count and check for channel prompt
        if uid in user_data:
            user_data[uid]['transcription_count'] = user_data[uid].get('transcription_count', 0) + 1
            save_user_data()

            if user_data[uid]['transcription_count'] >= 5 and not user_data[uid]['channel_prompt_sent']:
                if not check_subscription(message.from_user.id):
                    send_subscription_message(message.chat.id)
                    user_data[uid]['channel_prompt_sent'] = True # Mark as sent
                    save_user_data()
...
def cleanup_old_data():
    ...
    for user_id, transcriptions in user_transcriptions.items():
        # Check if user_id exists in user_data before accessing last_activity
        if user_id in user_data and 'last_activity' in user_data[user_id]:
            last_activity = datetime.fromisoformat(user_data[user_id]['last_activity'])
            if last_activity < seven_days_ago:
                keys_to_delete_transcriptions.append(user_id)
        else:
            # If user_id is not in user_data or no last_activity, assume old and delete
            keys_to_delete_transcriptions.append(user_id)
...
def handle_text_for_tts_or_fallback(message):
    ...
    # If user has a voice selected for TTS (user_tts_mode[uid] holds the voice name)
    if user_tts_mode.get(uid):
        threading.Thread(
            target=lambda: asyncio.run(synth_and_send_tts(message.chat.id, uid, message.text))
        ).start()
    elif uid in tts_users:  # User has a voice saved, but maybe didn't start with /text_to_speech this session
        user_tts_mode[uid] = tts_users[uid]  # Reactivate TTS mode with the saved voice
        threading.Thread(
            target=lambda: asyncio.run(synth_and_send_tts(message.chat.id, uid, message.text))
        ).start()
    else:  # User hasn't selected a voice yet for TTS or is not in TTS mode
        bot.send_message(
            message.chat.id,
