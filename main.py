...
TTS_VOICES_BY_LANGUAGE = {
    ...
    "Oromo 🇪🇹": ["om-ET-WaseNeural", "om-ET-HawiNeural"], # Example voices, need actual ones if available
    "Persian 🇮🇷": ["fa-IR-FaribaNeural", "fa-IR-HamidNeural"],
    "Bosnian 🇧🇦": ["bs-BA-VesnaNeural", "bs-BA-SasaNeural"],
    "Bulgarian 🇧🇬": ["bg-BG-KalinaNeural", "bg-BG-IvanNeural"],
    "Catalan 🇪🇸": ["ca-ES-JoanaNeural", "ca-ES-EnricNeural"],
    "Icelandic 🇮🇸": ["is-IS-AsdisNeural", "is-IS-JonNeural"],
    "Irish 🇮🇪": ["ga-IE-OrlaithNeural", "ga-IE-SeanNeural"],
    "Macedonian 🇲🇰": ["mk-MK-MariaNeural", "mk-MK-FilipNeural"],
    "Maltese 🇲🇹": ["mt-MT-GraceNeural", "mt-MT-JosefNeural"],
    ...
    "Galician 🇪🇸": ["gl-ES-BrendaNeural", "gl-ES-RoiNeural"],
    "Javanese 🇮🇩": ["jv-ID-ArdiNeural", "jv-ID-SitiNeural"],
    "Sundanese 🇮🇩": ["su-ID-JajangNeural", "su-ID-TutiNeural"],
    "Hausa 🇳🇬": ["ha-NG-FatimaNeural", "ha-NG-NasirNeural"],
    "Yoruba 🇳🇬": ["yo-NG-KemiNeural", "yo-NG-GbengaNeural"],
    "Igbo 🇳🇬": ["ig-NG-ChikaNeural", "ig-NG-ChineduNeural"],
    "Uzbek 🇺🇿": ["uz-UZ-MadinaNeural", "uz-UZ-SuhrobNeural"],
    "Azerbaijani 🇦🇿": ["az-AZ-BabekNeural", "az-AZ-BanuNeural"],
    "Kazakh 🇰🇿": ["kk-KZ-AigulNeural", "kk-KZ-NurzhanNeural"],
    "Kyrgyz 🇰🇬": ["ky-KG-AigulNeural", "ky-KG-BekturNeural"],
    "Tajik 🇹🇯": ["tg-TJ-SayaraNeural", "tg-TJ-FiruzNeural"],
    "Turkmen 🇹🇲": ["tk-TM-MaralNeural", "tk-TM-SohbetNeural"],
    "Mongolian 🇲🇳": ["mn-MN-NominNeural", "mn-MN-BatbayarNeural"],
    "Lao 🇱🇦": ["lo-LA-ChanthavongNeural", "lo-LA-KeomanyNeural"],
    "Nepali 🇳🇵": ["ne-NP-SaritaNeural", "ne-NP-AbhisekhNeural"],
    "Sinhala 🇱🇰": ["si-LK-SameeraNeural", "si-LK-ThiliniNeural"],
    "Khmer 🇰🇭": ["km-KH-SreymomNeural", "km-KH-PannNeural"],
...
# Modified keep_typing for animated ellipsis
def keep_typing_animated(chat_id, message_id, stop_event):
    ellipses = ["", ".", "..", "..."]
    i = 0
    while not stop_event.is_set():
        try:
            bot.edit_message_text(
                ...
                text=f"Transcribing{ellipses[i % 4]}"
            )
            i += 1
            time.sleep(0.5) # Shorter delay for faster animation
        except telebot.apihelper.ApiTelegramException as e:
            if "message is not modified" not in str(e):
                logging.error(f"Error updating typing animation: {e}")
            break
        except Exception as e:
            logging.error(f"Error in typing animation thread: {e}")
            break
...
# --- DELETED: help_handler function ---

# --- DELETED: privacy_notice_handler function ---
...
def handle_file(message):
    ...
    status_msg = None
    try:
        status_msg = bot.send_message(message.chat.id, "Transcribing...") # Initial message for animation
    except Exception as e:
        logging.error(f"Error sending initial status message: {e}")
    ...
    # Start the animated typing thread
    if status_msg:
        typing_thread = threading.Thread(target=keep_typing_animated, args=(message.chat.id, status_msg.message_id, stop_typing))
    ...
    try:
        threading.Thread(
            ...
            args=(message, stop_typing, is_document_video, status_msg)
        ).start()
    except Exception as e:
...
def process_media_file(message, stop_typing, is_document_video, status_msg):
    """
    ...
    try:
        # The "Transcribing..." message is now handled by keep_typing_animated
        # if status_msg:
        #     try:
        #         bot.edit_message_text(
        #             chat_id=status_msg.chat.id,
        #             message_id=status_msg.message_id,
        #             text="🔍 Transcribing your file..."
        #         )
        #     except Exception as e:
        ...
        # --- REMOVED: "✅ Transcription complete! Total files processed:" message ---
        # The status message is now simply edited to "Done" or deleted.
        if status_msg:
            try:
...
LANGUAGES = [
    ...
    {"name": "Oromo", "flag": "🇪🇹", "code": "om"}, # Adding Oromo
    {"name": "Persian", "flag": "🇮🇷", "code": "fa"}, # Adding Persian (Iran)
]
