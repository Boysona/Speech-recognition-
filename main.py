...
# --- NEW: REQUIRED_CHANNEL for subscription check ---
REQUIRED_CHANNEL = "@transcriberbo" # Replace with your actual channel username (e.g., @MyAwesomeChannel)
...
# --- NEW: Subscription Check Functions ---
def check_subscription(user_id):
    try:
        member = bot.get_chat_member(REQUIRED_CHANNEL, user_id)
        return member.status in ['member', 'administrator', 'creator']
    except telebot.apihelper.ApiTelegramException as e:
        # User not found in channel, or channel not found, or other API error
        logging.warning(f"Error checking subscription for user {user_id} in {REQUIRED_CHANNEL}: {e}")
        return False

def send_subscription_message(chat_id):
    markup = telebot.types.InlineKeyboardMarkup()
    # Remove the "@" from the channel name for the URL
    channel_url = f"https://t.me/{REQUIRED_CHANNEL[1:]}"
    markup.add(
        telebot.types.InlineKeyboardButton("Click here to join the channel", url=channel_url)
    )
    bot.send_message(
        chat_id,
...
# --- NEW: Custom Filter for Channel Subscription ---
class ChatMemberFilter(telebot.custom_filters.AdvancedCustomFilter):
    key = 'is_subscribed'

    def check(self, message, config):
        user_id = message.from_user.id
        return check_subscription(user_id)

# Register the custom filter
bot.add_custom_filter(ChatMemberFilter())

# --- NEW: Handler for channel member updates ---
@bot.chat_member_handler(func=lambda chat_member: chat_member.chat.username == REQUIRED_CHANNEL[1:])
def on_chat_member_update(chat_member_update: telebot.types.ChatMemberUpdated):
    old_member = chat_member_update.old_chat_member
    new_member = chat_member_update.new_chat_member
    user_id = new_member.user.id
    chat_id = chat_member_update.chat.id # This is the channel's chat_id

    # We are interested in the user's private chat with the bot, not the channel chat_id
    # So we'll get the user's private chat_id from the user_id
    user_private_chat_id = user_id # For private chats, user_id is the chat_id

    if old_member.status in ['member', 'administrator', 'creator'] and new_member.status in ['left', 'kicked']:
        # User left the channel
        try:
            bot.send_message(
                user_private_chat_id,
                f"Oh no! It looks like you've left our channel {REQUIRED_CHANNEL}. 😔\n\n"
                "This bot won't work for you unless you're a member. "
                "Please rejoin the channel to continue using the bot. We'd love to have you back! 🥰"
            )
            # You might want to disable some features for this user
            # or store their state to prevent further processing
            logging.info(f"User {user_id} left {REQUIRED_CHANNEL}. Sent warning message.")
        except telebot.apihelper.ApiTelegramException as e:
            logging.error(f"Could not send left channel message to user {user_id}: {e}")
    elif old_member.status in ['left', 'kicked'] and new_member.status == 'member':
        # User rejoined the channel
        try:
            bot.send_message(
                user_private_chat_id,
                f"Welcome back to {REQUIRED_CHANNEL}! 🎉 You can now continue using the bot. 🥰"
            )
            logging.info(f"User {user_id} rejoined {REQUIRED_CHANNEL}.")
        except telebot.apihelper.ApiTelegramException as e:
            logging.error(f"Could not send rejoined channel message to user {user_id}: {e}")

# --- End of NEW Subscription Check Functions ---
...
# --- NEW: Use custom filter for subscription check on media messages ---
@bot.message_handler(content_types=['voice', 'audio', 'video', 'video_note'], is_subscribed=True)
...
# --- NEW: Handler for non-subscribed users trying to send media ---
@bot.message_handler(content_types=['voice', 'audio', 'video', 'video_note'], is_subscribed=False)
def handle_file_not_subscribed(message):
...
LANGUAGES = [
    ...
    {"name": "Finnish 🇫🇮": {"name": "Finnish", "flag": "🇫🇮", "code": "fi-FI"}},
...
@bot.message_handler(commands=['language'], is_subscribed=True)
...
@bot.message_handler(commands=['language'], is_subscribed=False)
def select_language_command_not_subscribed(message):
    send_subscription_message(message.chat.id)
...
@bot.message_handler(commands=['media_language'], is_subscribed=True)
...
@bot.message_handler(commands=['media_language'], is_subscribed=False)
def select_media_language_command_not_subscribed(message):
    send_subscription_message(message.chat.id)
...
def button_translate_handler(call):
    ...
    if not check_subscription(uid): # Check subscription here too for inline buttons
...
@bot.message_handler(commands=['translate'], is_subscribed=True)
...
@bot.message_handler(commands=['translate'], is_subscribed=False)
def handle_translate_not_subscribed(message):
    send_subscription_message(message.chat.id)

@bot.message_handler(commands=['summarize'], is_subscribed=True)
...
@bot.message_handler(commands=['summarize'], is_subscribed=False)
def handle_summarize_not_subscribed(message):
    send_subscription_message(message.chat.id)
...
# --- NEW: Handle all text messages for TTS after command selection (with subscription check) ---
@bot.message_handler(func=lambda message: message.content_type == 'text' and not message.text.startswith('/'), is_subscribed=True)
...
# --- NEW: Handler for non-subscribed users sending regular text messages ---
@bot.message_handler(func=lambda message: message.content_type == 'text' and not message.text.startswith('/'), is_subscribed=False)
def handle_text_not_subscribed(message):
    send_subscription_message(message.chat.id)

@bot.message_handler(func=lambda m: True, content_types=['photo', 'sticker', 'document'], is_subscribed=True)
...
@bot.message_handler(func=lambda m: True, content_types=['photo', 'sticker', 'document'], is_subscribed=False)
def fallback_non_text_or_media_not_subscribed(message):
    send_subscription_message(message.chat.id)
...
def webhook():
    ...
    if request.method == 'POST':
        ...
        if content_type and content_type.startswith('application/json'):
            ...
            # Process chat_member_update explicitly
...
def set_webhook_route():
    # Make sure to set allowed_updates for chat_member to be effective
    bot.set_webhook(url=WEBHOOK_URL, allowed_updates=["message", "edited_message", "callback_query", "chat_member"])
...
if __name__ == "__main__":
    ...
    # Register the custom filter with the bot after it's defined
    bot.add_custom_filter(telebot.custom_filters.ChatMemberFilter())
