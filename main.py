...
def handle_file(message):
    ...
    try:
        status_msg = bot.send_message(message.chat.id, "⚙️ Processing your file...")
    except Exception as e:
...
def process_media_file(message, stop_typing, is_document_video, status_msg):
    ...
    try:
        # EDIT status message to "🔍 Transcribing..."
        if status_msg:
            try:
                bot.edit_message_text(
        ...
        # Edit status message to "✅ Done" and display total count, then send transcription
        if status_msg:
            try:
                bot.edit_message_text(
                    chat_id=status_msg.chat.id,
                    message_id=status_msg.message_id,
                    text=f"✅ Transcription complete! Total files processed: {total_files_processed}"
                )
            except Exception as e:
                logging.error(f"Error editing status message to Done: {e}")
        ...
        bot.send_message(
            message.chat.id,
            "😓𝗪𝗲’𝗿𝗲 𝘀𝗈𝗋𝗋𝘆, 𝗮𝗇 𝗲𝗋𝗋𝗈𝗋 𝗼𝗰𝗰𝘂𝗋𝗋𝗂𝗇𝗀 𝗱𝘂𝗿𝗂𝗇𝗀 𝘁𝗋𝗮𝗇𝘀𝗂𝗉𝗍𝗂𝗈𝗇.\n"
