...
# --- BOT CONFIGURATION ---
TOKEN = "7790991731:AAHZks7W-iUwp6pcKD56eOeq3wduPjAiwow" # Replace with your actual bot token
...
# Download directory (only used temporarily if in-memory becomes difficult)
...
# Admin uptime message storage (No longer used for uptime message, but keeping for other potential admin states)
...
# Function to update uptime message (No longer used for admin start, but kept if needed elsewhere)
...
# Clean up old user data (user_transcriptions, user_memory)
def clean_old_user_data():
    """
    Clean up old user data (transcriptions and memory) after 7 days.
    This should be added at bot startup or a regular schedule.
    """
    ...
    # Repeat this, e.g., every 24 hours
...
def start_handler(message):
    ...
    # The admin uptime message logic is removed for both admin and regular users
...
"""
    ...
    # Admin specific keyboard if needed, otherwise removed as per request
    if message.from_user.id == ADMIN_ID:
        keyboard = telebot.types.ReplyKeyboardMarkup(resize_keyboard=True)
        keyboard.add("Send Broadcast", "Total Users", "/status")
        bot.send_message(message.chat.id, "Admin options:", reply_markup=keyboard)
...
def keep_typing(chat_id, stop_event):
    """This sends a 'typing' action continuously until the stop_event is set."""
    while not stop_event.is_set():
        try:
            bot.send_chat_action(chat_id, 'typing')
            time.sleep(4) # Telegram requires sending every 5 seconds
        except Exception as e:
    ...
    try:
        ...
        if uid not in user_media_language_settings:
            bot.send_message(chat_id,
                             "⚠️ Please first select the audio file language using /media_language before sending the file.")
            return
        ...
        if file_obj.file_size > FILE_SIZE_LIMIT:
            bot.send_message(chat_id, "The file you sent exceeds the allowed size limit (maximum 20MB).")
            return
        ...
        try:
            ...
            bot.send_message(chat_id, "😓 Sorry, your file could not be converted to the correct format for speech recognition. Please try another file.")
            return
        ...
        if len(transcription) > 4000:
            ...
            with open(fn, 'rb') as doc:
                bot.send_document(
                    ...
                    caption="Here is your transcription. Click the buttons below for more options."
                )
            os.remove(fn)
        else:
            bot.reply_to(
        ...
        # Attractive error handling
        error_message = "😓 Sorry, an unexpected error occurred while processing your file."
        if "Bad audio" in str(e) or "empty data" in str(e):
            error_message = "😓 Sorry, the audio in your file is unclear or incomplete. Please try a file with clear audio."
        elif "No transcription" in str(e) or "cannot understand" in str(e):
            error_message = "😓 Sorry, I couldn't understand the audio content. Please ensure the audio is clear and your language setting is correct."
        bot.send_message(chat_id, error_message)
    finally:
        stop_typing_event.set() # Stop typing action
        # Clean up temporary files if any (in-memory doesn't need cleanup)
        # They are already deleted if disk is used.
...
def select_language_command(message):
    ...
    bot.send_message(
        message.chat.id,
        "Please choose your preferred language for future **translations and summaries**:",
        reply_markup=markup
    )
...
def select_media_language_command(message):
    ...
    bot.send_message(
        message.chat.id,
        "Please select the language of the audio files you want me to transcribe. This helps ensure accurate transcription.",
        reply_markup=markup
    )
...
def callback_set_media_language(call):
    ...
    bot.edit_message_text(
        chat_id=call.message.chat.id,
        message_id=call.message.message_id,
        text=f"✅ Your media transcription language has been set to: **{lang}**",
        parse_mode="Markdown"
    )
...
def button_translate_handler(call):
    ...
    if preferred_lang:
        ...
        bot.edit_message_text(
            chat_id=call.message.chat.id,
            message_id=call.message.message_id,
            text="Please choose the language you want to translate to:",
            reply_markup=markup
        )
...
def button_summarize_handler(call):
    ...
    if preferred_lang:
        ...
        bot.edit_message_text(
            chat_id=call.message.chat.id,
            message_id=call.message.message_id,
            text="Please choose the language you want the summary to be in:",
            reply_markup=markup
        )
    bot.answer_callback_query(call.id)
...
def callback_translate_to(call):
    ...
    # Update the previous message to show translation in progress
    bot.edit_message_text(
        chat_id=call.message.chat.id,
        message_id=call.message.message_id,
...
def callback_summarize_in(call):
    ...
    # Update the previous message to show summarization in progress
    bot.edit_message_text(
        chat_id=call.message.chat.id,
        message_id=call.message.message_id,
...
def do_translate_with_saved_lang(message, uid, lang, message_id):
    ...
    if translated.startswith("Error:"):
        bot.send_message(message.chat.id, f"Please try again. An error occurred during translation. Error: {translated}")
        return
...
def do_summarize_with_saved_lang(message, uid, lang, message_id):
    ...
    if summary.startswith("Error:"):
        bot.send_message(message.chat.id, f"Please try again. An error occurred during summarization. Error: {summary}")
        return
...
def handle_translate(message):
    ...
    if preferred_lang:
        ...
        bot.send_message(
            message.chat.id,
            "Please choose the language you want to translate to:",
            reply_markup=markup
        )
...
def handle_summarize(message):
    ...
    if preferred_lang:
        ...
        bot.send_message(
            message.chat.id,
            "Please choose the language you want the summary to be in:",
            reply_markup=markup
        )
...
def transcribe_audio_chunks_in_memory(audio_bytes_io: BytesIO, lang_code: str) -> str | None:
    ...
    try:
        ...
        while start_ms < total_length_ms:
            ...
            chunk_in_memory.seek(0) # Reset stream position
