...
processing_semaphore = threading.Semaphore(5)  # Xad u samee 5 hawlood isku mar ah
...
def handle_file(message):
    ...
    # --- BEGIN: Acquire semaphore to limit concurrent processing ---
    if not processing_semaphore.acquire(blocking=False):
        bot.send_message(message.chat.id, "Waan ka xunnahay, bot-ku hadda wuu mashquulsan yahay, fadlan isku day markii danbe,")
        return
    # --- END: Acquire semaphore ---
    ...
    try:
        ...
        # Haddii thread-ka uusan bilaaban, sii tuur semaphore-ka
...
def process_media_file(message, stop_typing, is_document_video):
    ...
    try:
        ...
        # --- RELEASE semaphore mark ---, si uu user kale u bilaabo
        processing_semaphore.release()
