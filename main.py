...
import telebot
import json
import subprocess # Changed: Replaced os.system with subprocess
...
import threading
import time
import io # Changed: For in-memory audio handling
...
# Webhook URL configuration (Moved for prominence)
WEBHOOK_URL_BASE = "https://speech-recognition-6i0c.onrender.com" # Replace with your actual Render URL
...
user_memory = {}
user_transcriptions = {}
# Dictionary to store messages that are currently being processed, to enable typing indicator
processing_message_ids = {} # Added: For managing typing indicator threads
...
# Added: Typing indicator function
def keep_typing(chat_id, stop_event):
    """
    Sends 'typing' chat action every few seconds until stop_event is set.
    """
    while not stop_event.is_set():
        try:
            bot.send_chat_action(chat_id, 'typing')
            time.sleep(4) # Telegram recommends sending action every 5 seconds
        except Exception as e:
            logging.error(f"Error in typing thread: {e}")
            break
...
I'm Media To text Bot. I help you save time by transcribing and summarizing voice messages, audio messages, and video notes. Simply send or forward a message to me. Send /help for more information.
"""
        )
...
4.  **Data Retention:**
    ...
    * **Transcriptions:** Held temporarily in the bot's active memory for immediate use, and cleared after 7 days or on new media processing.
...
def handle_file(message):
    ...
    if file_obj.file_size > FILE_SIZE_LIMIT:
        return bot.send_message(message.chat.id, "😓 Sorry, the file size you uploaded is too large (max allowed is 20MB). Please try with a smaller file.")
    ...
    typing_thread.start()
    processing_message_ids[message.chat.id] = stop_typing # Store stop event for this chat
    ...
    # Using BytesIO for in-memory handling where possible, but ffmpeg needs paths for complex operations
    # For simplicity and given the user's `os.system` usage, we'll keep temporary disk files for ffmpeg for now
    # and ensure cleanup. A full in-memory pipeline with pydub and sr.AudioData would be more complex.
    downloaded_file_path = os.path.join(DOWNLOAD_DIR, f"{uuid.uuid4()}{os.path.splitext(info.file_path)[1]}")
    ...
    try:
        ...
        # Changed: Using BytesIO for in-memory WAV buffer
        wav_buffer = io.BytesIO()
        try:
            # Changed: Using subprocess for ffmpeg conversion
            command = [
                ffmpeg.get_ffmpeg_exe(),
                '-i', downloaded_file_path,
                '-vn', # no video
                '-acodec', 'pcm_s16le', # PCM signed 16-bit little-endian
                '-ar', '16000', # 16kHz audio
                '-ac', '1', # mono audio
                '-f', 'wav', # output format
                'pipe:1' # output to stdout
            ]
            process = subprocess.run(command, capture_output=True, check=True)
            wav_buffer.write(process.stdout)
            wav_buffer.seek(0) # Reset buffer position

            if wav_buffer.getbuffer().nbytes == 0:
                raise Exception("FFmpeg conversion resulted in empty WAV data.")

        except subprocess.CalledProcessError as e:
            logging.error(f"FFmpeg conversion failed: {e.stderr.decode()}")
            bot.send_message(message.chat.id, "😓 Sorry, there was an issue converting your file for transcription. Please check the audio quality and try again.")
            return # Exit function on error
        except Exception as e:
            logging.error(f"FFmpeg conversion failed: {e}")
            bot.send_message(message.chat.id, "😓 Sorry, your file cannot be converted to the correct voice recognition format. Please ensure it's a valid audio/video file.")
            return # Exit function on error


        media_lang_code = get_lang_code(user_media_language_settings[uid])
        if not media_lang_code:
            bot.send_message(message.chat.id, f"❌ The language *{user_media_language_settings[uid]}* does not have a valid code. Please re-select the language for media transcription using /media_language.")
            return # Exit function on error

        # Changed: Pass BytesIO object directly
        transcription = transcribe_audio_chunks_in_memory(wav_buffer, media_lang_code) or ""
        ...
        bot.send_message(message.chat.id, "😓 Sorry, an unexpected error occurred during transcription. The audio might be unclear or the file corrupted. Please try again.")
    finally:
        # Stop typing indicator
        if message.chat.id in processing_message_ids:
            processing_message_ids[message.chat.id].set()
            del processing_message_ids[message.chat.id]

        # Clean up downloaded file
        if os.path.exists(downloaded_file_path):
            os.remove(downloaded_file_path)
            logging.info(f"Cleaned up {downloaded_file_path}")
        
        # Clean up old transcriptions and memory (every 7 days)
        # This needs a more robust, asynchronous background task or a simpler check
        # For demonstration, a simple check on every file processing for the current user's data
        now = datetime.now()
        for user_id_key, last_activity_str in list(user_data.items()): # Iterate over a copy
            last_activity = datetime.fromisoformat(last_activity_str)
            if now - last_activity > timedelta(days=7):
                if user_id_key in user_transcriptions:
                    del user_transcriptions[user_id_key]
                    logging.info(f"Cleared old transcriptions for user {user_id_key}")
                if user_id_key in user_memory:
                    del user_memory[user_id_key]
                    logging.info(f"Cleared old chat memory for user {user_id_key}")
                # You might also want to remove users who haven't been active for a very long time
                # del user_data[user_id_key] # Only if you want to completely remove inactive users
...
def do_translate_with_saved_lang(message, uid, lang, message_id):
    ...
    if translated.startswith("Error:"):
        bot.send_message(message.chat.id, f"😓 Sorry, there was an issue translating: {translated}. Please try again later.")
        return
...
def do_summarize_with_saved_lang(message, uid, lang, message_id):
    ...
    if summary.startswith("Error:"):
        bot.send_message(message.chat.id, f"😓 Sorry, there was an issue summarizing: {summary}. Please try again later.")
        return
...
# Changed: Audio chunking size to 20 seconds, using in-memory buffer
def transcribe_audio_chunks_in_memory(audio_buffer: io.BytesIO, lang_code: str) -> str | None:
    r = sr.Recognizer()
    full_transcription = []
    chunk_length_ms = 20000 # Changed: 20 seconds
    overlap_ms = 500

    try:
        # Load audio from in-memory buffer
        audio = AudioSegment.from_wav(audio_buffer)
        total_length_ms = len(audio)
        start_ms = 0

        logging.info(f"Starting chunking, total length {total_length_ms / 1000} seconds.")

        while start_ms < total_length_ms:
            end_ms = min(start_ms + chunk_length_ms, total_length_ms)
            chunk = audio[start_ms:end_ms]

            # Export chunk to a new in-memory buffer for SpeechRecognition
            chunk_buffer = io.BytesIO()
            chunk.export(chunk_buffer, format="wav")
            chunk_buffer.seek(0) # Reset buffer position
            ...
            with sr.AudioFile(chunk_buffer) as source:
                try:
                    ...
                    logging.warning(f"Speech Recognition could not understand audio in chunk {start_ms/1000}s - {end_ms/1000}s. This might be due to unclear audio or silence.")
                except sr.RequestError as e:
                    logging.error(f"Could not request results from Google Speech Recognition service; {e} for chunk {start_ms/1000}s - {end_ms/1000}s. Please check your internet connection or try again later.")
                except Exception as e:
...
def set_webhook_route(): # Renamed to avoid conflict with `set_webhook` function in main block
    bot.set_webhook(url=WEBHOOK_URL_BASE + WEBHOOK_URL_PATH)
    return f"Webhook set to {WEBHOOK_URL_BASE + WEBHOOK_URL_PATH}", 200
...
if __name__ == "__main__":
    # Clean up downloads directory on start
    ...
    bot.delete_webhook() # Always delete before setting to avoid issues
    bot.set_webhook(url=WEBHOOK_URL_BASE + WEBHOOK_URL_PATH)
