...
# === DIIWAANKA user_media_language_settings WAA LAGU TIRTIRAY ===
...
# === SAVE USER_MEDIA_LANGUAGE_SETTINGS IYO ISTICMAALKII WAA LAGU TIRTIRAY ===
...
def set_bot_info():
    commands = [
        ...
        #telebot.types.BotCommand("media_language", "📝Set language for media transcription"),  # Lagu tirtiray
...
1.  **Send a File for Transcription:**
    * Simply send a voice message, audio file, video note, or a video file (e.g. .mp4) as a document/attachment.
    * The bot will then process your media **automatically**, detect the language, and send back the transcribed text. If the transcription is very long, it will be sent as a text file for easier reading.
...
def handle_file(message):
    ...
    # === TIRTIRAY QAAB HADDII U AN FOOSO LUQAD DOORASHO INSHA ALLAAH ===
...
def process_media_file(message, stop_typing, is_document_video):
    ...
    try:
        ...
        # --- NEW: Transcribe using FasterWhisper with automatic language detection ---
        if WHISPER_MODEL is None:
            bot.send_message(message.chat.id, "❌ Speech recognition model is not loaded. Please contact support.")
            return

        # Run transcription without specifying language → automatic detection
        segments, info = WHISPER_MODEL.transcribe(wav_audio_path)
        # info.language waa lang code la ogaaday (tusaale: "so", "en", iwm)
        detected_lang = info.language  # Haddii loo baahdo, waad kaydin kartaa
        transcription = " ".join([segment.text for segment in segments]) or ""
        ...
        if len(transcription) > 4000:
            ...
            with open(fn, 'rb') as doc:
                bot.send_document(
                    ...
                    caption=f"Here’s your transcription (detected language: {detected_lang}). Tap a button below for more options."
                )
            os.remove(fn)
        else:
            bot.reply_to(
                message,
                f"{transcription}\n\n_(Lan detected: {detected_lang})_",
                reply_markup=buttons,
                parse_mode="Markdown"
            )
...
# --- Language Selection and Saving for Text Translations ---
LANGUAGES = [
...
def transcribe_audio_with_faster_whisper(audio_path: str, lang_code: str) -> str | None:
    # (Hadda hawshan looma baahna—codsiga Automatic detection ayaa toos loo isticmaalay)
    if WHISPER_MODEL is None:
