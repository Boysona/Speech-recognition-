...
def handle_file(message):
    ...
    # --- MODIFIED: Do NOT reset user_tts_mode here.
    # User might want to send a media file for transcription AND then send text for TTS.
    # The 'handle_text_for_tts_or_fallback' will handle if no voice is chosen.
    # user_tts_mode[uid] = False # REMOVED
    # --- END MODIFIED ---
    ...
    for user_id in keys_to_delete_tts_users:
        if user_id in tts_users:
            ...
            if user_id in user_tts_mode:
                del user_tts_mode[user_id] # REMOVED: This should not delete if user is still active and wants to keep TTS voice saved
...
# Duplicate code removed, only keep one instance of webhook and related functions.
