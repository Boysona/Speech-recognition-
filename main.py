...
# MongoDB URI (replace with your actual password and app name)
MONGODB_USERNAME = "hoskasii"
MONGODB_PASSWORD = "GHyCdwpI0PvNuLTg"
MONGODB_APP_NAME = "TELEGRAM BOT" # Use the app name provided in the request
MONGO_URI = f"mongodb+srv://{MONGODB_USERNAME}:{MONGODB_PASSWORD}@cluster0.dy7oe7t.mongodb.net/?retryWrites=true&w=majority&appName={MONGODB_APP_NAME}"
...
try:
    ...
    db = client.get_database("telegram_bot_db") # Get a database named 'telegram_bot_db'
    users_collection = db.users # Get a collection named 'users'
except Exception as e:
    print(f"Failed to connect to MongoDB: {e}")
    # Exit or handle this error appropriately, as the bot won't function without DB
    db = None
...
TOKEN = "7790991731:AAGpbz6nqE5f0Dvs6ZSTrRoR1LMrrf4rMqU"  # Replace with your actual bot token
...
# User-specific language settings for translate/summarize (still in-memory for now)
user_language_settings = {}
user_media_language_settings = {}

# --- NEW: TTS User settings and Voices (still in-memory for now) ---
tts_users = {}
...
def update_user_activity(user_id):
    if users_collection is not None:  # Fix applied here
...
def increment_transcription_count(user_id):
    if users_collection is not None:  # Fix applied here
        users_collection.update_one(
            {"_id": user_id},
...
def start_handler(message):
    ...
    update_user_activity(user_id) # Update last_active timestamp in DB

    # Retrieve user data from MongoDB
    user_doc = get_user_data_from_db(user_id)
    if not user_doc:
        # If user not found, insert with initial transcription_count
        if users_collection is not None: # Fix applied here
            users_collection.insert_one({
                "_id": user_id,
                "last_active": datetime.now(),
                "transcription_count": 0,
                "preferred_language": None, # Initialize with None
                "media_language": None, # Initialize with None
                "tts_voice": None # Initialize with None
            })
    else:
        # Load in-memory settings from DB if they exist
        if user_doc.get("preferred_language"):
            user_language_settings[str(user_id)] = user_doc["preferred_language"]
        if user_doc.get("media_language"):
            user_media_language_settings[str(user_id)] = user_doc["media_language"]
        if user_doc.get("tts_voice"):
            tts_users[str(user_id)] = user_doc["tts_voice"]
...
def help_handler(message):
    ...
    user_doc = get_user_data_from_db(user_id)
    transcription_count = user_doc.get('transcription_count', 0) if user_doc else 0

    # --- MODIFIED: Check subscription only if transcription count is met ---
    if transcription_count >= 5 and not check_subscription(user_id):
...
def status_handler(message):
    ...
    total_registered_users = get_total_users_from_db()
    
    # Calculate active users today from MongoDB
    today = datetime.now().date()
    active_today_count = 0
    if users_collection is not None:  # Fix applied here
        active_today_count = users_collection.count_documents(
            {"last_active": {"$gte": datetime.combine(today, datetime.min.time()),
...
def total_users(message):
    bot.send_message(message.chat.id, f"Total registered users: {get_total_users_from_db()}")
...
def handle_file(message):
    ...
    # If user hasn't chosen a media_language yet, prompt them (now from in-memory)
    if str(user_id) not in user_media_language_settings:
        bot.send_message(
            message.chat.id,
...
def process_media_file(message, stop_typing, is_document_video):
    ...
    user_id = message.from_user.id # Use integer user_id for DB operations
    ...
    try:
        ...
        media_lang_name = user_media_language_settings.get(str(user_id)) # Get from in-memory
        ...
        user_transcriptions.setdefault(str(user_id), {})[message.message_id] = transcription
        ...
        # --- MODIFIED: Increment transcription count for the user in MongoDB ---
        increment_transcription_count(user_id)
        # --- END MODIFIED ---
        ...
        user_doc_after_inc = get_user_data_from_db(user_id)
        current_transcription_count = user_doc_after_inc.get('transcription_count', 0) if user_doc_after_inc else 0
        if current_transcription_count == 5 and not check_subscription(user_id):
            send_subscription_message(message.chat.id)
        # --- END MODIFIED ---

    except Exception as e:
        logging.error(f"Error processing file for user {user_id}: {e}")
        try:
            bot.set_message_reaction(chat_id=message.chat.id, message_id=message.message_id, reaction=[])
        except Exception as remove_e:
...
def on_tts_voice_change(call):
    ...
    tts_users[str(user_id)] = voice # Update in-memory
    ...
    user_tts_mode[str(user_id)] = voice
...
async def synth_and_send_tts(chat_id, user_id, text):
    voice = tts_users.get(str(user_id), "en-US-AriaNeural") # Get from in-memory
...
def callback_set_language(call):
    ...
    user_language_settings[str(user_id)] = lang # Update in-memory
...
def callback_set_media_language(call):
    ...
    lang = parts[1]
    user_media_language_settings[str(user_id)] = lang # Update in-memory
...
def button_translate_handler(call):
    ...
    if str(user_id) not in user_transcriptions or message_id not in user_transcriptions[str(user_id)]:
        bot.answer_callback_query(call.id, "❌ No transcription found for this message.")
        return

    preferred_lang = user_language_settings.get(str(user_id)) # Get from in-memory
    if preferred_lang:
        bot.answer_callback_query(call.id, "Translating with your preferred language...")
        threading.Thread(target=do_translate_with_saved_lang, args=(call.message, str(user_id), preferred_lang, message_id)).start()
    else:
...
def button_summarize_handler(call):
    ...
    if preferred_lang:
        ...
        threading.Thread(target=do_summarize_with_saved_lang, args=(call.message, str(user_id), preferred_lang, message_id)).start()
    else:
...
def callback_translate_to(call):
    ...
    if message_id:
        threading.Thread(target=do_translate_with_saved_lang, args=(call.message, str(user_id), lang, message_id)).start()
    else:
        if str(user_id) in user_transcriptions and call.message.reply_to_message and call.message.reply_to_message.message_id in user_transcriptions[str(user_id)]:
            threading.Thread(target=do_translate_with_saved_lang, args=(call.message, str(user_id), lang, call.message.reply_to_message.message_id)).start()
        else:
...
def callback_summarize_in(call):
    ...
    if message_id:
        threading.Thread(target=do_summarize_with_saved_lang, args=(call.message, str(user_id), lang, message_id)).start()
    else:
        if str(user_id) in user_transcriptions and call.message.reply_to_message and call.message.reply_to_message.message_id in user_transcriptions[str(user_id)]:
            threading.Thread(target=do_summarize_with_saved_lang, args=(call.message, str(user_id), lang, call.message.reply_to_message.message_id)).start()
        else:
...
def do_translate_with_saved_lang(message, uid_str, lang, message_id):
    original = user_transcriptions.get(uid_str, {}).get(message_id, "")
    if not original:
    ...
    translated = ask_gemini(uid_str, prompt)
...
def do_summarize_with_saved_lang(message, uid_str, lang, message_id):
    original = user_transcriptions.get(uid_str, {}).get(message_id, "")
    if not original:
    ...
    bot.send_chat_action(message.chat.id, 'typing')
    summary = ask_gemini(uid_str, prompt)
...
def handle_translate(message):
    ...
    if not message.reply_to_message or str(user_id) not in user_transcriptions or message.reply_to_message.message_id not in user_transcriptions[str(user_id)]:
        return bot.send_message(message.chat.id, "❌ Please reply to a transcription message to translate it.")

    transcription_message_id = message.reply_to_message.message_id
    preferred_lang = user_language_settings.get(str(user_id)) # Get from in-memory
    if preferred_lang:
        threading.Thread(target=do_translate_with_saved_lang, args=(message, str(user_id), preferred_lang, transcription_message_id)).start()
    else:
...
def handle_summarize(message):
    ...
    preferred_lang = user_language_settings.get(str(user_id)) # Get from in-memory
    if preferred_lang:
        threading.Thread(target=do_summarize_with_saved_lang, args=(message, str(user_id), preferred_lang, transcription_message_id)).start()
    else:
...
def handle_text_for_tts_or_fallback(message):
    ...
    if user_tts_mode.get(str(user_id)):  # If user has a voice selected for TTS
        threading.Thread(
            target=lambda: asyncio.run(synth_and_send_tts(message.chat.id, user_id, message.text))
        ).start()
    elif str(user_id) in tts_users:  # User has a voice saved, but maybe didn't start with /text_to_speech
        user_tts_mode[str(user_id)] = tts_users[str(user_id)]  # Reactivate TTS mode with the saved voice
        threading.Thread(
            target=lambda: asyncio.run(synth_and_send_tts(message.chat.id, user_id, message.text))
        ).start()
...
def cleanup_old_data():
    """Cleans up user_transcriptions and user_memory older than 7 days, and inactive users from MongoDB."""
    seven_days_ago = datetime.now() - timedelta(days=7)

    # Clean up in-memory transcriptions
    keys_to_delete_transcriptions = []
    for user_id_str, transcriptions in user_transcriptions.items():
        user_id = int(user_id_str) # Convert back to int for DB lookup
        user_doc = get_user_data_from_db(user_id)
        if user_doc and 'last_active' in user_doc:
            last_activity = user_doc['last_active']
            if last_activity < seven_days_ago:
                keys_to_delete_transcriptions.append(user_id_str)
        else:
            keys_to_delete_transcriptions.append(user_id_str) # Consider old if no activity data in DB

    for user_id_str in keys_to_delete_transcriptions:
        if user_id_str in user_transcriptions:
            del user_transcriptions[user_id_str]
            logging.info(f"Cleaned up old in-memory transcriptions for user {user_id_str}")

    # Clean up in-memory chat memory
    keys_to_delete_memory = []
    for user_id_str in user_memory:
        user_id = int(user_id_str) # Convert back to int for DB lookup
        user_doc = get_user_data_from_db(user_id)
        if user_doc and 'last_active' in user_doc:
            last_activity = user_doc['last_active']
            if last_activity < seven_days_ago:
                keys_to_delete_memory.append(user_id_str)
        else:
            keys_to_delete_memory.append(user_id_str) # Consider old if no activity data in DB

    for user_id_str in keys_to_delete_memory:
        if user_id_str in user_memory:
            del user_memory[user_id_str]
            logging.info(f"Cleaned up old in-memory chat memory for user {user_id_str}")

    # Clean up in-memory TTS user preferences
    keys_to_delete_tts_users = []
    for user_id_str in tts_users:
        user_id = int(user_id_str) # Convert back to int for DB lookup
        user_doc = get_user_data_from_db(user_id)
        if user_doc and 'last_active' in user_doc:
            last_activity = user_doc['last_active']
            if last_activity < seven_days_ago:
                keys_to_delete_tts_users.append(user_id_str)
        else:
            keys_to_delete_tts_users.append(user_id_str) # Consider old if no activity data in DB

    for user_id_str in keys_to_delete_tts_users:
        if user_id_str in tts_users:
            del tts_users[user_id_str]
            if user_id_str in user_tts_mode:
                del user_tts_mode[user_id_str]
            logging.info(f"Cleaned up old in-memory TTS preferences for user {user_id_str}")

    # MongoDB cleanup: Remove users from DB who have not been active for 7 days
    if users_collection is not None: # Fix applied here
        result = users_collection.delete_many({"last_active": {"$lt": seven_days_ago}})
        logging.info(f"MongoDB cleanup: Deleted {result.deleted_count} inactive users.")
