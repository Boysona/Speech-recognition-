...
WEBHOOK_URL = "https://speech-recognition-8c66.onrender.com"
...
# --- CACHING: transcription cache file ---
...
def save_transcription_cache():
    with open(cache_file, 'w') as f:
        json.dump(transcription_cache, f, indent=2)
...
def process_media_file(message, stop_typing):
    ...
    # 1) **Check Cache**: haddii file_id horey transcription loo sameeyey, soo celi isla transcription-ka
    if file_obj.file_id in transcription_cache:
        transcription = transcription_cache[file_obj.file_id]
        logging.info(f"Cache hit for file_id {file_obj.file_id}")
    else:
        ...
        try:
            ...
            # 2) **Transcribe with Retry**: isku day ilaa 3 jeer haddii RequestError yimaado
            transcription = transcribe_audio_from_bytes_with_retry(wav_audio_data, media_lang_code, retries=3)

            if transcription is None:
                # Haddii dhammaan isku dayadii ay fashilmaan
                try:
                    bot.set_message_reaction(chat_id=message.chat.id, message_id=message.message_id, reaction=[]) # Remove reaction
                except Exception as remove_e:
                    logging.error(f"Error removing reaction on transcription failure: {remove_e}")
                bot.send_message(message.chat.id,
                                 "😓 Sorry, I couldn’t understand the audio after several attempts. Please try with a clearer recording.")
                return

            # 3) **Save into Cache** si markale loo isticmaalo
            transcription_cache[file_obj.file_id] = transcription
    ...
    # **Halkan transcription ayaa diyaar ah** (iyada oo laga yaabo in laga helay cache-ka ama cusub została)
...
# ====== **Yeni Function: transcribe with retry** ======
def transcribe_audio_from_bytes_with_retry(audio_bytes: bytes, lang_code: str, retries: int = 3) -> str | None:
    """
    Transcribe audio in-memory with up to `retries` isku day on RequestError.
    Qeybinta (chunking) waa sidee loo qabtay hore. Haddii chunk gaar ahi fahmi kari waayo, ayaa
    iskuday mar kale.
    """
    r = sr.Recognizer()
    full_transcription = []
            ...
            with sr.AudioFile(chunk_io) as source:
                ...
                while attempt < retries and not success:
                    try:
                        ...
                        logging.info(f"Transcribed chunk {start_ms/1000}s–{end_ms/1000}s: {text[:50]}...")
                        success = True
                    except sr.UnknownValueError:
                        logging.warning(f"Chunk not understood at {start_ms/1000}s–{end_ms/1000}s")
                        # No retry on UnknownValueError; move on
                        break
                    except sr.RequestError as e:
                        attempt += 1
                        logging.error(f"RequestError (attempt {attempt}/{retries}) for chunk {start_ms/1000}s–{end_ms/1000}s: {e}")
                        time.sleep(1)  # yar yar dib u dhig
                    except Exception as e:
                        logging.error(f"Unexpected error on chunk {start_ms/1000}s–{end_ms/1000}s: {e}")
                        break
