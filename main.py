...
# Download directory (still used for intermediate files)
...
def process_media_file(message, stop_typing, is_document_video):
    ...
    convert it to WAV, run SpeechRecognition transcription (in chunks based on duration),
    ...
    try:
        ...
        # --- NEW: Transcribe using SpeechRecognition in chunks based on duration ---
        ...
        transcription = transcribe_audio_with_dynamic_chunks(wav_audio_path, media_lang_code) or ""
...
# --- UPDATED: Transcribe in dynamic chunks (20s if >1min, else 5s) with hash and original format info ---
def transcribe_audio_with_dynamic_chunks(audio_path: str, lang_code: str) -> str | None:
    ...
    try:
        ...
        if total_duration_ms > 60_000:  # more than 1 minute
            chunk_length_ms = 20_000  # 20 seconds
        else:
            chunk_length_ms = 5_000   # 5 seconds

        # Iterate over chunks
        for i in range(0, total_duration_ms, chunk_length_ms):
            chunk = sound[i:i + chunk_length_ms]
            # Create a temporary filename with original format info and hash
            temp_chunk_filename = os.path.join(
                DOWNLOAD_DIR,
                f"chunk_{uuid.uuid4()}_{i // 1000}_{(i + chunk_length_ms) // 1000}.wav"
            )
            # Export the chunk to a temporary WAV file for transcription
            chunk.export(temp_chunk_filename, format="wav")

            # Calculate SHA256 hash of the chunk file
            with open(temp_chunk_filename, "rb") as f:
                file_bytes = f.read()
                sha256_hash = hashlib.sha256(file_bytes).hexdigest()

            # Rename the chunk file to include the hash
            base, ext = os.path.splitext(temp_chunk_filename)
            hashed_filename = f"{base}_{sha256_hash}{ext}"
            os.rename(temp_chunk_filename, hashed_filename)

            # Use SpeechRecognition to transcribe this chunk
            with sr.AudioFile(hashed_filename) as source:
            ...
            # Clean up the hashed chunk file after transcribing
            os.remove(hashed_filename)

        return text.strip()

    except Exception as e:
        logging.error(f"Error during dynamic chunked transcription: {e}")
        return None
...
def cmd_text_to_speech(message):
    ...
    update_user_activity(user_id)
    # --- MODIFIED: Check subscription only if transcription count is met ---
    if user_data.get(user_id, {}).get('transcription_count', 0) >= 5 and not check_subscription(message.chat.id):
        send_subscription_message(message.chat.id)
        return
