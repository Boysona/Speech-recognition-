...
def process_media_file(message, stop_typing, is_document_video):
    ...
    wav_audio_path = None
    slowed_wav_audio_path = None # New path for slowed audio
    ...
    try:
        ...
        # --- NEW: Slow down the audio to 0.7x before transcribing ---
        slowed_wav_audio_path = os.path.join(DOWNLOAD_DIR, f"slowed_{uuid.uuid4()}.wav")
        try:
            audio = AudioSegment.from_wav(wav_audio_path)
            slowed_audio = audio.set_frame_rate(int(audio.frame_rate * 0.7)) # Adjust frame rate to slow down
            # You can also use audio.speedup(playback_speed=0.7) for a different kind of speed change,
            # but adjusting frame rate is generally more direct for simple slowing.
            slowed_audio.export(slowed_wav_audio_path, format="wav")
            if not os.path.exists(slowed_wav_audio_path) or os.path.getsize(slowed_wav_audio_path) == 0:
                raise Exception("Pydub slowing failed or resulted in an empty file.")
        except Exception as e:
            logging.error(f"Error slowing down audio with pydub: {e}")
            try:
                bot.set_message_reaction(chat_id=message.chat.id, message_id=message.message_id, reaction=[])
            except Exception as remove_e:
                logging.error(f"Error removing reaction on pydub error: {remove_e}")
            bot.send_message(
                message.chat.id,
                "😓 Sorry, there was an issue processing the audio for speech recognition (slowing down). Please try again."
            )
            return

        # --- NEW: Transcribe using SpeechRecognition in 20-second chunks, using the SLOWED audio ---
        ...
        # Pass the slowed audio path to the chunking function
        transcription = transcribe_audio_with_chunks(slowed_wav_audio_path, media_lang_code) or ""
        ...
        if slowed_wav_audio_path and os.path.exists(slowed_wav_audio_path):
            os.remove(slowed_wav_audio_path)
            logging.info(f"Cleaned up {slowed_wav_audio_path}")
