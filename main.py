...
# --- BOT CONFIGURATION ---
# Waxaan rajeynayaa inaad ku dartay token-kaaga iyo API Key-gaaga Render environment variables.
# Haddii kale, ku beddel "YOUR_BOT_TOKEN_HERE" iyo "YOUR_GEMINI_API_KEY_HERE"
TOKEN = os.environ.get("BOT_TOKEN", "7790991731:AAH4rt8He_PABDa28xgcY3dIQwmtuQD-qiM")  # Replace with your actual bot token if not using env var
ADMIN_ID = 5978150981  # Replace with your actual Admin ID
# Webhook URL - Replace with your actual Render URL
WEBHOOK_URL = os.environ.get("RENDER_EXTERNAL_HOSTNAME", "https://speech-recognition-9j3f.onrender.com")
...
try:
    ...
    logging.info("FasterWhisper 'tiny' model loaded successfully.")
...
def get_tts_user_voice(uid):
    return tts_users.get(str(uid), "en-US-AriaNeural") # Default to a specific voice if not found
...
if not GEMINI_API_KEY:
    logging.error("GEMINI_API_KEY environment variable not set. Gemini features will not work.")

def ask_gemini(user_id, user_message):
    if not GEMINI_API_KEY:
        return "Error: Gemini API Key is not set. Please contact the bot administrator."
    ...
    try:
        ...
        return f"Error: Could not connect to Gemini API. Please try again later. ({e})"
    except json.JSONDecodeError as e:
        logging.error(f"Error decoding Gemini API response: {e}")
        return "Error: Failed to parse Gemini API response. Please try again later."
...
def update_uptime_message(chat_id, message_id):
    ...
    while True:
        try:
            ...
            # Handle "message is not modified" (expected) and "message not found" (handle by creating new)
            if "message is not modified" not in str(e):
                logging.error(f"Error updating uptime message for message_id {message_id}: {e}")
                # If message not found, we can try to reset admin_uptime_message for this admin
                # This depends on how start_handler logic handles existing vs. new messages
            break # Break the loop if there's an error (e.g., message deleted)
        except Exception as e:
...
def start_handler(message):
    ...
    if message.from_user.id == ADMIN_ID:
        ...
        # --- IMPROVED ADMIN UPTIME MESSAGE HANDLING ---
        # Try to edit existing message or create new one
        existing_info = admin_uptime_message.get(ADMIN_ID)
        message_to_edit_id = None
        sent_message = None # Initialize sent_message

        if existing_info and existing_info.get('message_id'):
            try:
                # Attempt to edit the existing message
                bot.edit_message_text(
                    chat_id=existing_info['chat_id'],
                    message_id=existing_info['message_id'],
                    text="Admin Panel and Uptime (updating live)...",
                    reply_markup=keyboard
                )
                message_to_edit_id = existing_info['message_id']
            except telebot.apihelper.ApiTelegramException as e:
                logging.warning(f"Failed to edit existing uptime message for admin {ADMIN_ID}: {e}. Sending new one.")
                # If editing fails (e.g., message not found), send a new one
                sent_message = bot.send_message(message.chat.id, "Admin Panel and Uptime (updating live)...", reply_markup=keyboard)
                message_to_edit_id = sent_message.message_id
        else:
            # If no existing info or message_id, send a new one
            sent_message = bot.send_message(message.chat.id, "Admin Panel and Uptime (updating live)...", reply_markup=keyboard)
            message_to_edit_id = sent_message.message_id

        # Start or restart uptime thread
        with admin_uptime_lock:
            # Ensure any old thread for this admin is stopped if it's still alive
            if existing_info and existing_info.get('thread') and existing_info['thread'].is_alive():
                # In a more robust system, you'd signal the old thread to stop.
                # For simplicity here, we let it terminate itself on error, or just run until the new one takes over.
                pass 

            admin_uptime_message[ADMIN_ID] = {'message_id': message_to_edit_id, 'chat_id': message.chat.id}
            uptime_thread = threading.Thread(target=update_uptime_message, args=(message.chat.id, message_to_edit_id))
...
def process_media_file(message, stop_typing):
    ...
    try:
        ...
        try:
            ...
            # Use Popen to prevent blocking and allow checking returncode
            ...
            logging.error(f"FFmpeg conversion failed: Return Code: {e.returncode}, Stdout: {e.stdout.decode()}, Stderr: {e.stderr.decode()}")
            try:
        ...
        media_lang_name = user_media_language_settings.get(uid) # Get the language name
        if not media_lang_name: # Handle case where language is not set
            bot.send_message(message.chat.id,
                             "⚠️ Transcription failed: Media language was not set. Please use /media_language to set it first.")
            return
...
def handle_text_for_tts_or_fallback(message):
    ...
    if user_tts_mode.get(uid):
        # We need to run the async function in a new event loop for threading compatibility
...
def webhook():
    # Health check for Render or other services
    ...
    if request.method == 'POST':
        ...
        if content_type and content_type.startswith('application/json'):
            try:
                ...
                return '', 200 # Always return 200 OK for successful processing
            except Exception as e:
                logging.error(f"Error processing Telegram update: {e}")
                return '', 200 # Telegram expects 200 OK even if your processing failed
        logging.warning(f"Received non-JSON POST request: {request.headers.get('Content-Type')}")
        return abort(403) # Not a valid Telegram webhook request

    return abort(403) # Other methods not allowed


@app.route('/set_webhook', methods=['GET', 'POST'])
def set_webhook_route():
    try:
        # Construct the full webhook URL including the RENDER_EXTERNAL_HOSTNAME from environment
        # If RENDER_EXTERNAL_HOSTNAME is not set, it defaults to the hardcoded WEBHOOK_URL
        full_webhook_url = os.environ.get("RENDER_EXTERNAL_HOSTNAME", WEBHOOK_URL)
        if full_webhook_url and not full_webhook_url.startswith("http"):
            full_webhook_url = f"https://{full_webhook_url}" # Ensure it's HTTPS

        bot.set_webhook(url=full_webhook_url)
        logging.info(f"Manually set webhook to {full_webhook_url}")
        return f"Webhook manually set to {full_webhook_url}", 200
    except Exception as e:
...
if __name__ == "__main__":
    ...
    # --- DEJI WEBHOOK-KA BILOWGA APPLICATION-KA ---
    # Tani waa qaybta ugu muhiimsan ee hagaajineysa khaladka "URL-ka bannaan"
    try:
        # Render wuxuu si toos ah u bixiyaa RENDER_EXTERNAL_HOSTNAME env var
        # Waxaanu isticmaalaynaa tan si loo dhiso URL-ka webhook-ka ee saxda ah.
        # Haddii uusan Render ahayn, waxaanu ku tiirsanaanaynaa WEBHOOK_URL.
        full_webhook_url = os.environ.get("RENDER_EXTERNAL_HOSTNAME", WEBHOOK_URL)
        if full_webhook_url and not full_webhook_url.startswith("http"):
            full_webhook_url = f"https://{full_webhook_url}" # Ensure it's HTTPS

        bot.set_webhook(url=full_webhook_url)
        logging.info(f"Webhook set successfully to {full_webhook_url} on startup.")
    except Exception as e:
        logging.error(f"Failed to set webhook on startup: {e}")
    # ----------------------------------------------

    # Bilaw server-ka Flask
