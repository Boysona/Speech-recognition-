...
# ── BOT CONFIGURATION ───────────────────────────────────────────────────────────
TOKEN = "7790991731:AAH4rt8He_PABDa28xgcY3dIQwmtuQD-qiM"
ADMIN_ID = 5978150981
...
firebase_admin.initialize_app(firebase_cert, {
    'databaseURL': 'https://tm-bot-data-base-default-rtdb.firebaseio.com/'
})

# Note: The provided code had the SERVICE_ACCOUNT_JSON_STRING and Firebase initialization duplicated.
# I've kept only one instance for correctness. If you intended two separate Firebase app initializations,
# please clarify. Otherwise, one is sufficient and correct.
...
def cleanup_old_data():
    ...
    for user_id, trans in user_transcriptions.items():
        ...
        if not last_activity:
            ...
            try:
                if datetime.fromisoformat(last_activity) < seven_days_ago:
                    to_delete_t.append(user_id)
            except:
                to_delete_t.append(user_id)
