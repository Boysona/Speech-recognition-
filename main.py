...
# Download directory (used only temporarily if in-memory is difficult)
...
# Clean up old user data (transcriptions and memory)
def clean_old_user_data():
    """
    Clean up user data older than 7 days.
    """
    ...
    for user_id, transcriptions in user_transcriptions.items():
        # If needed, implement timestamps for each transcription to delete individually.
        pass
        ...
        with admin_uptime_lock:
            if ADMIN_ID in admin_uptime_message and admin_uptime_message[ADMIN_ID].get('stop_event'):
                ...
                if admin_uptime_message[ADMIN_ID].get('thread') and admin_uptime_message[ADMIN_ID]['thread'].is_alive():
                    admin_uptime_message[ADMIN_ID]['thread'].join(timeout=2)
            ...
            uptime_thread = threading.Thread(
                ...
                args=(message.chat.id, sent_message.message_id, stop_event)
            )
...
def keep_typing(chat_id, stop_event):
    ...
    while not stop_event.is_set():
        try:
            ...
            time.sleep(4)  # Telegram requires an action every 5 seconds
        except Exception as e:
...
def process_media_file_threaded(message):
    ...
    try:
        ...
        if uid not in user_media_language_settings:
            bot.send_message(
                chat_id,
                "⚠️ Please select the audio file language first using /media_language before sending the file."
            )
            return
        ...
        if file_obj.file_size > FILE_SIZE_LIMIT:
            bot.send_message(
                chat_id,
                "The file you sent exceeds the allowed size limit (max 20MB)."
            )
            return
        ...
        try:
            ...
            bot.send_message(
                chat_id,
                "😓 Sorry, your file could not be converted to a format suitable for speech recognition. Please try a different file."
            )
            return
        ...
        if not media_lang_code:
            bot.send_message(
                chat_id,
                f"❌ Language *{user_media_language_settings[uid]}* does not have a valid code. Please reselect the language."
            )
            return
        ...
        if "Bad audio" in str(e) or "empty data" in str(e):
            error_message = "😓 Sorry, the audio of your file is unclear or incomplete. Please try a clear audio file."
        elif "No transcription" in str(e) or "cannot understand" in str(e):
            error_message = "😓 Sorry, I could not understand the content of your audio. Please ensure the audio is clear and the language is correct."
...
def select_language_command(message):
    ...
    bot.send_message(
        message.chat.id,
        "Please choose your preferred language for **translations and summaries** going forward:",
        reply_markup=markup
    )
...
def select_media_language_command(message):
    ...
    bot.send_message(
        message.chat.id,
        "Please choose the language of the audio files you need me to transcribe. This helps ensure accurate transcription.",
        reply_markup=markup
    )
...
def callback_set_media_language(call):
    ...
    bot.edit_message_text(
        chat_id=call.message.chat.id,
        message_id=call.message.message_id,
        text=f"✅ The media transcription language has been set to: **{lang}**",
        parse_mode="Markdown"
    )
...
def button_summarize_handler(call):
    ...
    if preferred_lang:
        ...
        bot.edit_message_text(
            chat_id=call.message.chat.id,
            message_id=call.message.message_id,
            text="Please choose the language you want the summary in:",
            reply_markup=markup
        )
    bot.answer_callback_query(call.id)
...
def do_translate_with_saved_lang(message, uid, lang, message_id):
    ...
    if not original:
        bot.send_message(
            message.chat.id,
            "❌ No transcription available for this message to translate."
        )
        return

    prompt = (
        f"Translate the following text into {lang}. Provide only the translated text, "
        "with no additional notes, explanations, or introductory/concluding remarks:\n\n"
        f"{original}"
    )
    ...
    if translated.startswith("Error:"):
        bot.send_message(
            message.chat.id,
            f"Please try again. An error occurred during translation. Here is the error: {translated}"
        )
        return
    ...
    if len(translated) > 4000:
        ...
        with open(fn, 'rb') as doc:
            bot.send_document(
                message.chat.id,
                doc,
                caption=f"Translation into {lang}",
                reply_to_message_id=message_id
            )
        os.remove(fn)
    else:
...
def do_summarize_with_saved_lang(message, uid, lang, message_id):
    ...
    if not original:
        bot.send_message(
            message.chat.id,
            "❌ No transcription available for this message to summarize."
        )
        return

    prompt = (
        f"Summarize the following text in {lang}. Provide only the summarized text, "
        "with no additional notes, explanations, or different versions:\n\n"
        f"{original}"
    )
    ...
    if summary.startswith("Error:"):
        bot.send_message(
            message.chat.id,
            f"Please try again. An error occurred during summarization. Here is the error: {summary}"
        )
        return
...
def handle_translate(message):
    ...
    if not message.reply_to_message or uid not in user_transcriptions or message.reply_to_message.message_id not in user_transcriptions[uid]:
        return bot.send_message(
            message.chat.id,
            "❌ Please reply to a transcription message to translate."
        )
...
def handle_summarize(message):
    ...
    if not message.reply_to_message or uid not in user_transcriptions or message.reply_to_message.message_id not in user_transcriptions[uid]:
        return bot.send_message(
            message.chat.id,
            "❌ Please reply to a transcription message to summarize."
        )
    ...
    if preferred_lang:
        ...
        bot.send_message(
            message.chat.id,
            "Please choose the language you want the summary in:",
            reply_markup=markup
        )
...
def fallback(message):
    ...
    if message.text and message.text.startswith('/'):
        ...
        bot.send_message(
            message.chat.id,
            "Please only send voice messages, audio, or videos to transcribe."
        )
