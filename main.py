...
def connect_to_mongodb():
    global mongo_client, db, users_collection, language_settings_collection, media_language_settings_collection, tts_users_collection
    try:
        ...
        # Exit or handle the error appropriately, perhaps with a retry mechanism
        exit(1) # Or raise an exception to stop the bot from running without DB

# Initialize collections (now handled by database operations directly)
# user_data is now fetched and updated directly via MongoDB
# user_language_settings is now fetched and updated directly via MongoDB
# user_media_language_settings is now fetched and updated directly via MongoDB
# tts_users is now fetched and updated directly via MongoDB

# User-specific language settings for translate/summarize (no longer loaded from file)
# user_language_settings = {}
# user_media_language_settings = {}
# tts_users = {} # No longer loaded from file
...
# Removed local JSON file saving functions
# def save_user_data():
...
# --- NEW MongoDB functions for data persistence ---
...
def get_user_media_language_setting_db(user_id):
    ...
    return doc.get("media_language") if doc else None
...
def get_tts_user_voice_db(uid):
    ...
    return doc.get("voice", "en-US-AriaNeural") if doc else "en-US-AriaNeural"
...
def set_bot_info():
    ...
    # --- REMOVED: Telegram short description and full description settings ---
...
def start_handler(message):
    ...
    # Check if user exists in DB, if not, insert with initial data
    ...
    if not existing_user:
        ...
        # Handle existing users without the new field
...
def status_handler(message):
    ...
    active_today = users_collection.count_documents({
        'last_active': {"$gte": today.isoformat()} # Query for dates starting from today
    })
...
def broadcast_message(message):
    ...
    success = fail = 0
    # Iterate through all user IDs in the users collection
...
def process_media_file(message, stop_typing, is_document_video):
    ...
    try:
        ...
        if not media_lang_name: # Should not happen if check above works, but for safety
        ...
        # --- MODIFIED: Increment transcription count for the user via DB ---
...
def handle_text_for_tts_or_fallback(message):
    ...
    # Check if a voice is saved in the DB, and activate TTS mode
...
def cleanup_old_data():
    ...
    # Cleanup old user data in MongoDB (only if they are inactive)
    # We should only clean up user data if there's no activity and they haven't used the bot for a long time.
    # For this implementation, we'll assume we keep all user IDs for stats, but just clean up their preferences
    # and reset transcription count if they become inactive for a very long time,
    # or you can choose to remove them entirely if that fits your privacy policy.
    # For now, let's keep user IDs for overall user count, but clear associated settings/counts if truly old.

    # Example: reset transcription count for very old inactive users (e.g., > 30 days)
    # You might adjust this logic based on your retention policy
    ...
    # Cleanup old language and media language settings
    language_settings_collection.delete_many(
        {'_id': {'$nin': [user_id for user_id in users_collection.find({}, {'_id': 1})]}} # delete if user_id doesn't exist in users_collection
    )
    media_language_settings_collection.delete_many(
        {'_id': {'$nin': [user_id for user_id in users_collection.find({}, {'_id': 1})]}}
    )
    tts_users_collection.delete_many(
        {'_id': {'$nin': [user_id for user_id in users_collection.find({}, {'_id': 1})]}}
    )
