...
import io  # Added to handle in-memory byte streams
...
# Storage for "reaction" messages
reaction_messages = {}  # { original_file_message_id: (chat_id, reaction_message_id) }
...
# Helper to delete a previously sent "reaction" message
def try_delete_reaction(original_msg_id):
    """
    If a reaction was stored for this original message, delete it.
    """
    if original_msg_id in reaction_messages:
        chat_id, reaction_id = reaction_messages.pop(original_msg_id)
        try:
            bot.delete_message(chat_id, reaction_id)
        except Exception as e:
            logging.error(f"Failed to delete reaction message: {e}")
...
def handle_file(message):
    ...
    # ─── ① Send a "👀" reaction-like reply ────────────────────────────────────────────
    try:
        reaction_msg = bot.reply_to(message, "👀")
        reaction_messages[ message.message_id ] = (message.chat.id, reaction_msg.message_id)
    except Exception as e:
    ...
    try:
        ...
        try_delete_reaction(message.message_id)
...
def process_media_file(message, stop_typing):
    ...
    try:
        ...
        try:
            ...
            # Delete the "👀" reaction before sending the error
            try_delete_reaction(message.message_id)
            bot.send_message(message.chat.id,
        ...
        # ─── Delete the "👀" reaction before sending the result ──────────────────────────
        try_delete_reaction(message.message_id)
