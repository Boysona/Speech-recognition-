...
# --- BOT CONFIGURATION (Using Media Transcriber Bot's Token) ---
...
bot = telebot.TeleBot(TOKEN, threaded=True) # threaded=True allows background tasks like typing actions
app = Flask(__name__) # Keep Flask for the UptimeRobot keep-alive endpoint
...
def main():
    ...
    # Start Flask app in a separate thread for keep-alive
    flask_thread = threading.Thread(target=run_flask_app)
    flask_thread.daemon = True # Allow main program to exit even if thread is still running
    flask_thread.start()

    logging.info("Starting bot polling...")
    bot.polling(none_stop=True) # Start the Telegram bot polling
