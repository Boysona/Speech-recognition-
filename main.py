...
# --- BOT CONFIGURATION (Using Media Transcriber Bot's Token, Polling Mode) ---
TOKEN = "7790991731:AAGpbz6nqE5f0Dvs6ZSTdRoR1LMrrf4rMqU"  # Beddel halkan haddii token-kaagu is badalay
ADMIN_ID = 5978150981  # Beddel halkan haddii Admin ID-gaagu is badalay
...
GEMINI_API_KEY = "AIzaSyAto78yGVZobxOwPXnl8wCE9ZW8Do2R8HA"  # Beddel halkan haddii API key-gaagu is badalay
...
# --- NEW: Check Channel Subscription (Conditional) ---
...
def generate_language_keyboard(callback_prefix, message_id=None):
    ...
    for lang in LANGUAGES:
        ...
        buttons.append(telebot.types.InlineKeyboardButton(f"{lang['name']} {lang['flag']}", callback_data=cb_data))
...
def make_tts_language_keyboard():
    ...
    for lang_name in TTS_VOICES_BY_LANGUAGE.keys():
        buttons.append(telebot.types.InlineKeyboardButton(lang_name, callback_data=f"tts_lang|{lang_name}"))
    for i in range(0, len(buttons), 3):
        markup.add(*buttons[i:i+3])
    return markup
...
def make_tts_voice_keyboard_for_language(lang_name):
    ...
    for voice in voices:
        markup.add(telebot.types.InlineKeyboardButton(voice, callback_data=f"tts_voice|{voice}"))
    markup.add(telebot.types.InlineKeyboardButton("⬅️ Back to Languages", callback_data="tts_back_to_languages"))
    return markup
...
# ────────────────────────────────────────────────────────────────────────────────────────────
#   K E E P – A L I V E   (Flask Web Server for UptimeRobot)
# ────────────────────────────────────────────────────────────────────────────────────────────
...
def start_polling_and_server():
    connect_to_mongodb()
    set_bot_info()
    cleanup_old_data()

    # Start polling in a separate thread
    polling_thread = threading.Thread(target=bot.infinity_polling, kwargs={'skip_pending': True})
