...
import time
import subprocess
import speech_recognition as sr  # Using SpeechRecognition instead of FasterWhisper
...
# --- Mock database for chunk metadata (per transcription session) ---
# This simply stores metadata so each chunk appears to come from a "new database"
# Structure: { "<transcription_id>": { "<chunk_filename>": {"db_id": <uuid>, "hash": <sha256>} } }
mock_db_store = {}
...
def process_media_file(message, stop_typing, is_document_video):
    ...
    convert it to WAV, run SpeechRecognition transcription in dynamic chunks,
    ...
    try:
        ...
        # --- NEW: Transcribe using SpeechRecognition in dynamic chunks (30s if >1min, else 10s) ---
        ...
        # Create a unique ID for this transcription session to store chunk metadata
        transcription_id = str(uuid.uuid4())
        mock_db_store[transcription_id] = {}

        transcription = transcribe_audio_with_dynamic_chunks(
            wav_audio_path,
            media_lang_code,
            transcription_id
        ) or ""
        ...
        bot.send_message(
            message.chat.id,
            "😓𝗪𝗲’𝗿𝗲 𝘀𝗼𝗿𝗿𝘆, 𝗮𝗻 𝗲𝗿𝗿𝗼𝗿 𝗼𝗰𝗰𝘂𝗿𝗲𝗱 𝗱𝘂𝗿𝗶𝗻𝗴 𝘁𝗿𝗮𝗻𝘀𝗰𝗿𝗶𝗽𝘁𝗶𝗼𝗻.\n"
...
# --- UPDATED: Transcribe in dynamic chunks (30s if >1min, else 10s) using pydub + SpeechRecognition ---
def transcribe_audio_with_dynamic_chunks(audio_path: str, lang_code: str, transcription_id: str) -> str | None:
    ...
    try:
        ...
        if total_duration_ms > 60_000:  # longer than 1 minute
        ...
        for i in range(0, total_duration_ms, chunk_length_ms):
            ...
            chunk_filename = os.path.join(
                DOWNLOAD_DIR,
                f"{uuid.uuid4()}_{i // 1000}_{min((i + chunk_length_ms) // 1000, total_duration_ms // 1000)}.wav"
            )
            # Export the chunk to a temporary file with unique tags (to simulate new format/db)
            db_id = str(uuid.uuid4())  # Mock database ID for this chunk
            # Compute SHA256 hash of raw chunk data
            raw_data = chunk.raw_data
            sha_hash = hashlib.sha256(raw_data).hexdigest()
            # Store in mock database
            mock_db_store.setdefault(transcription_id, {})[chunk_filename] = {
                "db_id": db_id,
                "hash": sha_hash
            }
            # Export with tags embedded (so each chunk has different metadata)
        ...
        logging.error(f"Error during dynamic chunk transcription: {e}")
        return None
...
def do_summarize_with_saved_lang(message, uid, lang, message_id):
    ...
    if len(summary) > 4000:
        ...
        bot.send_message(chat_id=message.chat.id, summary, reply_to_message_id=message_id)
...
def cleanup_old_data():
    ...
    # --- NEW: Also clean up mock_db_store entries if older than 7 days ---
    # We don't have timestamps per chunk, so skip detailed cleanup for now or implement as needed
